...
7. **Iterative Roadmap Execution Protocol**:
   When requested to execute roadmaps or goals, follow the 7-step loop: (1) Implement target phase tasks; (2) Debug until all unit tests pass; (3) Run **comparative benchmarks after EACH phase and subphase** (**50 episodes** for full phases, **25 episodes** for subphases) comparing directly against the prior baseline state (`baseline` vs `contender`) and testing across all active subversion branches (`version/v0.8.1`, etc.) to evaluate metric dynamics and universality; (4) Evaluate decision nodes; (5) Synchronize documentation under `agent_notes/`; (6) Synchronize cross-subversion branches (`version/v0.8.1`, `version/v0.9.1`, etc.); (7) Commit locally and request push approval.
