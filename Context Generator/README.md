...
**Context Generator** is a Python-based urban 3D context generator designed for solar, shading, daylighting, and building environmental simulations. It extracts real 100m urban context scenes from OpenStreetMap (OSM) across diverse urban typologies (Strict Grid vs. Organic/Relaxed) or procedurally synthesizes realistic 3D building contexts and central testing site boundaries.

---

## Key Features

1. **OSM Urban Context Harvester (`osm_extractor.py`)**:
   - Queries OpenStreetMap (Overpass API) for 3D building footprints, heights, and levels.
   - Extracts a **100m radius** context patch centered on a target location.
   - Identifies the central building, carves it out as the **Testing Site Boundary**, and retains surrounding buildings as the **3D Urban Context**.
...
4. **3D Metrics & Interactive WebGL Viewer (`geometry_3d.py`, `visualizer.py`)**:
   - Computes **Site Area ($m^2$)**, **Floor Area Ratio (FAR)**, **Ground Coverage Ratio (GCR)**, **Avg/Max Height**, and **Sky View Factor (SVF)** proxy.
   - Generates interactive 3D WebGL preview `.html` files in browser with color gradients by height and highlighted Gold site parcel.

5. **Multi-Format 3D Simulation Exporters (`exporter.py`, `blender_bridge.py`)**:
   - Exports standard 3D Wavefront `.obj` files for Radiance, Ladybug, PySolar, Rhinoceros/Grasshopper, and EnergyPlus.
...
```bash
blender --background --python blender_bridge.py -- output/nyc_midtown_context.json
```
