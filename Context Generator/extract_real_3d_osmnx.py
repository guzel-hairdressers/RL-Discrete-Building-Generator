...
def extract_city_osmnx_3d(city_key, radius_m=160.0):
    ...
    # 1. Fetch Buildings and Building Parts via OSMnx
    try:
        # Fetch both 'building' and 'building:part' for detailed 3D massings
    ...
    for idx, row in gdf_bldgs.iterrows():
        ...
        # Convert Polygon/MultiPolygon coordinates to local meters relative to (ref_lat, ref_lon)
        polys = []
        ...
        for poly in polys:
            ...
            if height is None or height <= 0:
                # Estimate height based on footprint area and city typology
                area = poly.area * 111000 * 111000  # approx m²
                if area > 2000:
    ...
    # Filter out giant block boundary wrapper if present
    # Pick central building as site boundary
    ...
    metrics = compute_urban_metrics(site_verts, context_buildings[1:])
