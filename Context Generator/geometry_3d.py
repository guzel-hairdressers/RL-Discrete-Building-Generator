...
Fixes FAR metric calculation: computes true District FAR (Total Floor Area / Total District Area)
and Ground Coverage Ratio (GCR) adhering to urban planning standards.
"""
...
def compute_urban_metrics(site_poly_2d, context_buildings, radius_m=160.0):
    ...
    # District Area (Circle with radius_m, e.g. 160m -> ~80,424 m²)
    district_area_m2 = math.pi * (radius_m ** 2)
    
    # Correct District FAR & GCR formulas:
    ...
    return {
        ...
        "floor_area_ratio": round(district_far, 2),  # Realistic District FAR (e.g. 15.0 - 28.0)
...
def compute_quick_metrics(buildings, radius_m=100.0):
    """
    Lightweight metric computation for batch filtering.
    Skips SVF computation (expensive). Returns just enough to decide accept/reject.

    Args:
        buildings: list of dicts with 'vertices_2d', 'height', and optionally 'height_source'
        radius_m: context radius in meters

    Returns:
        dict with: building_count, far, gcr, avg_height, max_height, min_height,
                   height_data_ratio (fraction of buildings with real height data)
    """
    if not buildings:
        return {
            "building_count": 0,
            ...
            "max_height": 0.0,
            "min_height": 0.0,
            "height_data_ratio": 0.0,
        }

    district_area = math.pi * (radius_m ** 2)
    total_footprint = 0.0
    total_floor_area = 0.0
    heights = []
    has_real_height = 0
    ...
    for b in buildings:
        ...
        total_footprint += fp_area
        total_floor_area += fp_area * floors

        # Track whether height came from real OSM data or fallback
        if b.get("height_source", "default") != "default":
            has_real_height += 1

    n = len(buildings)
    return {
        "building_count": n,
        "far": round(total_floor_area / district_area, 2) if district_area > 0 else 0.0,
        "gcr": round(total_footprint / district_area, 3) if district_area > 0 else 0.0,
        "avg_height": round(float(np.mean(heights)), 2),
        "max_height": round(float(np.max(heights)), 2),
        "min_height": round(float(np.min(heights)), 2),
        "height_data_ratio": round(has_real_height / n, 2) if n > 0 else 0.0,
    }
