...
from osm_extractor import extract_city_context
...
def handle_extract(args):
    ...
    if args.all:
        ...
        # Default target
        cities_to_extract = ["nyc_midtown", "barcelona_eixample", "tokyo_shinjuku"]

    for c_key in cities_to_extract:
        scene = extract_city_context(c_key, radius=args.radius)
        
        # Render 3D WebGL & Export OBJ
        html_out = os.path.join(OUTPUT_DIR, f"{c_key}_3d.html")
        obj_out = os.path.join(OUTPUT_DIR, f"{c_key}_3d.obj")
        json_out = os.path.join(OUTPUT_DIR, f"{c_key}_context.json")
...
def main():
    ...
    extract_parser.add_argument("--radius", type=float, default=100.0, help="Context radius in meters (default 100.0)")

    # Command 2: Generate Procedural
    ...
    gen_parser.add_argument("--radius", type=float, default=100.0, help="Context radius in meters (default 100.0)")
    gen_parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducibility")

    # Command 3: List cities
    ...
    if args.command == "extract":
        ...
        # Default fallback action: run synthetic generator and list cities
        parser.print_help()
