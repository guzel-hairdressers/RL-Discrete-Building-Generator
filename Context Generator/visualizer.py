...
window.addEventListener('message', (event) => {
  ...
  const matCore = new THREE.MeshLambertMaterial({ color: 0xee7258 });
  const matRoom = new THREE.MeshLambertMaterial({ color: 0x9bc2a7 });
  const matSpecial = new THREE.MeshLambertMaterial({ color: 0x649880 });
  const matCorridor = new THREE.MeshLambertMaterial({ color: 0xebb952 });
  const edgeMat = new THREE.LineBasicMaterial({ color: 0x1e293b, linewidth: 1.2 });
  ...
  placements.forEach((placement) => {
    ...
    geom.translate(0, floorIdx * floorHeight + 0.2, 0);
