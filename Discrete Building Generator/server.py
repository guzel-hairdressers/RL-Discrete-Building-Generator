"""Module Lab v0.7-D PyTorch training and WebSocket server.
...
def select_device() -> torch.device:
    """Choose CUDA, with CPU fallback for fast ARM64 scalar RL policy execution."""
...
class FloorEnvironment:
    ...
    def _candidate_from_anchor(
        ...
        site_bounds = G.bounds_of(self.site["outer"])
    ...
    def generate_candidates_for_module(
        ...
        if placing_first:
            ...
            allowed_cats = ["room"]
            if not single_floor and core_count < 2:
        ...
        for category in allowed_cats:
            for rotation in rotations:
                ...
                if profiler: profiler.record("cgAnchorSearch", time.perf_counter() - t_anchor)
    ...
    def _candidate_from_anchor(
        ...
        cells = G.rasterize_polygon(poly)
        valid_cells = cells and all(_cell_key(cell) in self.site["cellSet"] for cell in cells)
    ...
    def generate_candidates(
        ...
        if len(self.dictionary) < int(settings.get("dictCap", 10)):
            special_candidates.append(
                PlacementCandidate(
...
class ParallelTrainer:
    ...
    def _sample_custom_shape(
        ...
        num_edges_probs = torch.softmax(num_edges_logits, dim=0)
        num_edges_behavior = (1.0 - epsilon) * num_edges_probs + epsilon * torch.full_like(num_edges_probs, 1.0 / 2)
        ...
        is_step0 = any(not env.placements for env in environments) and not bool(settings.get("singleFloor"))
        ...
        for logit in edge_length_logits:
            ...
            edge_length_log_probs.append(dist.log_prob(action))
        ...
        for logit in angle_logits:
            ...
            angle_log_probs.append(dist.log_prob(action))
        ...
        for attempt in range(3):
            try:
                ...
                if is_step0 and float(candidate_module["area"]) + 1.0e-8 < 24.0 and attempt < 24:
                ...
                total_log_prob = num_edges_log_prob + torch.stack(edge_length_log_probs).sum() + torch.stack(angle_log_probs).sum()
                break
            except ValueError:
        ...
        if module is None or (is_step0 and float(module["area"]) + 1.0e-8 < 24.0):
            ...
            e0 = large_edge_indices[0]
            ang0 = G.ANGLE_PALETTE.index(90.0) if 90.0 in G.ANGLE_PALETTE else 0
            try:
                module = G.synthesize_custom_module(
                    settings, category, 4, [e0, e0, e0], [ang0, ang0], f"s{slot_index}"
                )
            except Exception:
                # Guaranteed valid rectangle fallback
                poly = [{"x": 0.0, "y": 0.0}, {"x": min_edge, "y": 0.0}, {"x": min_edge, "y": min_edge}, {"x": 0.0, "y": min_edge}]
                module = G._module_record(
                    identifier=f"s{slot_index}",
                    name="Custom Quad Fallback",
                    category=category,
                    poly=poly,
            ...
            total_log_prob = num_edges_log_prob
    ...
    def _finish_episode(self) -> dict[str, Any]:
        ...
        dict_limit = int(self.settings.get("dictionarySizeLimit", 10))
    ...
    def step(self, generation_id: Any, episode: Any) -> dict[str, Any]:
        ...
        for environment in self.environments:
            ...
            candidate_generation_duration = time.perf_counter() - generation_started
        ...
        for environment, candidates in candidate_groups:
            ...
            selected_candidate = candidates[int(selected_index.item())]
            ...
            if selected_candidate.module["id"] == "stop":
                ...
                for _attempt in range(2):
                    try:
                        ...
                        new_candidates = environment.generate_candidates_for_module(new_module, self.settings, orientation_basis, profiler=self.step_profiler)
                        if new_candidates:
                            self.shape_log_probs.append(shape_log_prob)
                            self.dictionary.append(new_module)
                            for env in self.environments:
                                if new_module not in env.dictionary:
                                    env.dictionary.append(new_module)
                                    env.module_uses[new_module["id"]] = 0

                            new_features = [cand.features for cand in new_candidates]
                            new_feature_tensor = torch.tensor(new_features, dtype=torch.float32, device=self.device)
                            new_logits = torch.nan_to_num(
                                self.model.placement_logits(new_feature_tensor), nan=0.0, posinf=20.0, neginf=-20.0
                            ).clamp(-30.0, 30.0) / temperature
                            new_dist = torch.distributions.Categorical(logits=new_logits)
                            new_selected = new_dist.sample()
                            
                            total_log_prob = distribution.log_prob(selected_index) + new_dist.log_prob(new_selected)
                            self.placement_log_probs.append(total_log_prob)
                            
                            selected_new_candidate = new_candidates[int(new_selected.item())]
                            placement = environment.place(selected_new_candidate)
                            placements.append(placement)
                            placed = True
                            break
                    except ValueError:
                        continue
                self.step_profiler.record("shapeSynthesis", time.perf_counter() - shape_synthesis_start)
                ...
                if not placed:
                    ...
                    if fallback_candidates:
                        ...
                        fb_logits = torch.nan_to_num(
                            self.model.placement_logits(fb_tensor), nan=0.0, posinf=20.0, neginf=-20.0
                        ).clamp(-30.0, 30.0) / temperature
                        fb_dist = torch.distributions.Categorical(logits=fb_logits)
                        fb_sel = fb_dist.sample()
                        self.placement_log_probs.append(fb_dist.log_prob(fb_sel))
                        placement = environment.place(fallback_candidates[int(fb_sel.item())])
                        placements.append(placement)
                    else:
                        self.placement_log_probs.append(distribution.log_prob(selected_index))
                        environment.done = True
        ...
        bpe_merge_start = time.perf_counter()
        layout_graphs = []
        for idx, environment in enumerate(self.environments):
            layout_graphs.append(graph.extract_layout_graph(environment.placements, idx))
        ...
        self.step_profiler.record("bpeMerge", time.perf_counter() - bpe_merge_start)
    ...
    def evaluate(self, generation_id: str, episode: int) -> dict[str, Any]:
        ...
        dict_limit_breach = max(0, prelim_vocab_size - dict_limit)
        breach_multiplier = 5.0 + 15.0 * min(1.0, float(self.episode) / 100.0)
        dict_breach_penalty = float(dict_limit_breach ** 2) * breach_multiplier

        score = float(metrics["score"]) + bpe_bonus - unmerged_triangle_penalty - dict_breach_penalty
        metrics["score"] = f"{score:.4f}"
        metrics["dictBreachPenalty"] = dict_breach_penalty
        metrics["dictLimitBreach"] = dict_limit_breach
        metrics["vocabSize"] = bpe_stats["unique_types"]
...
app = FastAPI(title="Module Lab v0.7-D")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
if __name__ == "__main__":
    print(f"Module Lab v0.7-D policy device: {select_device().type}")
