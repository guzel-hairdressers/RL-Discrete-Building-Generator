...
* **Key Achievements**:
  ...
  * **Stable A2C default + optional cosine/ratio-clip schedule** ([benchmark](benchmarks/v0.9.0-alpha_rl_refactor_2026-08-25.md) § LR schedule + ratio clip): flipped the default `learningRate` **0.003 → 0.001** (0.003 is a known A2C mid-run collapse at eps ~200–350; 0.001 reproduces the `free_wins` stability exactly). Added default-neutral LR-schedule + PPO-style ratio-clip settings — `lrSchedule`(constant|cosine|linear)/`lrFloor`/`lrHorizon`/`ratioClip` — plus override-only candidate caps `candidateCatLimit`/`candidateEdgeWindow`. Positively tested: **cosine 0.003→0.001 + `ratioClip=0.2`** (mean 41.30 / final-50 45.09, best in matrix, −26% wall); `candidateCatLimit=48` is the only cap with quality upside; edge-window widening is negative. Single-seed (42); multi-seed deferred.
  ...
  * **170 Unit Tests — 100% passing (skipped=2)** — the A2C refactor's 4 regressions resolved (26-08-26): real empty-episode `loss.backward()` crash fixed in `_learn_from_episode` (guarded on `loss.requires_grad`), stale `ppo_gae`/`_max_cores_for_site` assertions updated to the intentional A2C + loosened-core behavior (tracked in [`issues.md`](issues.md)).
