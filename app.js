...
(() => {
  ...
  function updateScoreBreakdown() {
    ...
    const topologyEnabled = metrics.topologyEnabled !== false;
    const topologyPenalty = Number(metrics.topologyPenalty) || 0;
    const topologyMultiplierValue = Number(metrics.topologyMultiplier);
    const topologyMultiplier = Number.isFinite(topologyMultiplierValue) ? topologyMultiplierValue : 0.08;
    ...
    if (topologyEnabled) {
      ...
      addRow('Core Topology', 'Disabled in canonical single-floor mode', '0.00 pts');
    }
    ...
    addDivider();
    addRow('Total Score', 'Raw Score - Topology Penalty + BPE Bonus - Triangle Penalty - Dict Penalty', `${totalScore.toFixed(4)} pts`, true);
  }
