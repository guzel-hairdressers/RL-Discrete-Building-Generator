...
def run_750_episode_convergence():
    num_episodes = 750
    ...
    print("750-EPISODE CONVERGENCE BENCHMARK: v0.8.4 (MLP) VS v0.8.5 (SE(2)-EQUIVARIANT TRANSFORMER)", flush=True)
    print(f"Configuration: {num_episodes} Sequential PPO Episodes, 120 Modules/Floor, 4 Parallel Floors, XL Lobed Sites", flush=True)
    print("=" * 115, flush=True)
    ...
    # -------------------------------------------------------------
    with open("benchmarks/convergence_750_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 115, flush=True)
    print("750-EPISODE CONVERGENCE SUMMARY: v0.8.4 MLP VS v0.8.5 SE(2) TRANSFORMER", flush=True)
    print("=" * 115, flush=True)
    print(f"{'Milestone':<15} | {'v0.8.4 MLP Score':<18} | {'v0.8.5 SE2 Score':<18} | {'v0.8.4 Fill':<14} | {'v0.8.5 Fill':<14} | {'Delta Score'}", flush=True)
    print("-" * 115, flush=True)
    for ckpt in (100, 250, 500, 750):
