#!/usr/bin/env python3
"""500-Episode Convergence Benchmark for fixed v0.8.6-a (Single-Episode Mini-Batch PPO).

Executes v0.8.6-a with identical mini-batch PPO architecture to v0.8.6-b, varying only the buffer size (target_episodes=1).
"""
...
def load_module(filepath: str, mod_name: str):
    spec = importlib.util.spec_from_file_location(mod_name, filepath)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load spec from {filepath}")
...
def main():
    path_86a = os.path.abspath("scratch/branches/server_v086a.py")
    with open(path_86a, "rb") as f:
        h = hashlib.md5(f.read()).hexdigest()
    print(f"Loading v0.8.6-a from {path_86a} (MD5: {h})")
    mod = load_module(path_86a, "server_v086a_clean")
    ...
    for ep in range(1, num_episodes + 1):
        ...
        if ep % 50 == 0 or ep in (1, 10, 20):
            ...
            print(
                f"[v0.8.6-a Fixed] Ep {ep:3d}/500: Last50 Score={avg:6.2f} pts | "
                f"Fill={avg_fill*100:4.1f}% | Mods={avg_mods:4.1f} | "
                f"Cur={s:6.2f} pts (fill={f*100:4.1f}%, mods={mc:2d}) | {elapsed/60:4.1f}m"
            )
    ...
    out_file = "benchmarks/convergence_v086a_fixed_500.json"
