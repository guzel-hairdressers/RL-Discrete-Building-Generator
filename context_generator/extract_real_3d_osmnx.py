...
# Configure reliable Overpass endpoint
ox.settings.overpass_url = "https://overpass.kumi.ai/api/interpreter"
...
def extract_city_osmnx_3d(city_key, radius_m=100.0, force_download=False):
    ...
    # Use cached local dataset first if available to avoid unnecessary network re-extractions
    if not force_download and os.path.exists(dataset_path):
        print(f"[OSMnx Cache] Loading saved 3D context dataset from: {dataset_path}")
        with open(dataset_path, "r") as f:
            context_scene = json.load(f)
        tier_str = str(context_scene.get("metrics", {}).get("areaTier", "l")).lower().replace("-", "")
        city_code = info.get("city_code", "nyc")
        html_out = os.path.join(OUTPUT_DIR, f"{tier_str}_{city_code}.html")
        create_3d_context_visualization(context_scene, html_out)
        print(f"[Render Success] Rendered 3D WebGL scene from local cache to: {html_out}")
        return
    ...
    try:
        ...
        print(f"[OSMnx Warning] Could not fetch live building features: {e}")
        if os.path.exists(dataset_path):
            print(f"[OSMnx Fallback] Loading saved 3D context dataset from: {dataset_path}")
            with open(dataset_path, "r") as f:
                context_scene = json.load(f)
            context_scene["radius_m"] = radius_m
    ...
    tier_str = str(metrics.get("areaTier", "l")).lower().replace("-", "")
    city_code = info.get("city_code", "nyc")
    html_out = os.path.join(OUTPUT_DIR, f"{tier_str}_{city_code}.html")
    create_3d_context_visualization(context_scene, html_out)
    print(f"[Render Success] Saved real 3D WebGL scene to: {html_out}")
