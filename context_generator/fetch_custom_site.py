...
os.makedirs(DIST_SITES_DIR, exist_ok=True)
...
def fetch_custom_site(lat: float, lon: float, custom_name: str = "Custom Site", custom_polygon: list = None, road_setback: float = 2.0, building_setback: float = 3.0, parcel_type: str = "convex_hull"):
    ...
    def _query_mirror(server_url):
        try:
            resp = requests.post(server_url, data={'data': query}, headers={'User-Agent': 'BuildingContextGenerator/1.0'}, timeout=(2.5, 7.0))
    ...
    osm_data = None
    print(f"[CustomFetcher] Querying {len(OVERPASS_SERVERS)} Overpass mirrors in fast parallel race...")
    ...
    # Crucial: cancel any pending queries and do NOT wait for slow servers to finish!
    ...
    dist_out_path = os.path.join(DIST_SITES_DIR, html_name)

    create_3d_context_visualization(scene_data, public_out_path)
    if os.path.exists(DIST_SITES_DIR) and public_out_path != dist_out_path:
        try:
            import shutil
            shutil.copy2(public_out_path, dist_out_path)
        except Exception:
            pass

    # Compute exact centered parcel polygon matching DATA.sitePerimeter in 3D visualizer
    site_poly = _clean_to_shapely(shifted_site_verts)
    if site_poly is not None:
        cx, cy = site_poly.centroid.x, site_poly.centroid.y
        centered_site_verts = [[round(p[0] - cx, 2), round(p[1] - cy, 2)] for p in list(site_poly.exterior.coords)]
    else:
        centered_site_verts = shifted_site_verts
    ...
    custom_record = {
        ...
        'polygon': [{'x': round(p[0], 2), 'y': round(p[1], 2)} for p in centered_site_verts],
    }
    ...
    if os.path.exists(os.path.dirname(DIST_CUSTOM_DS_PATH)):
        try:
            with open(DIST_CUSTOM_DS_PATH, 'w') as f:
                json.dump(custom_dataset, f, indent=2)
        except Exception:
            pass

    # Also update master data/real_sites_dataset.json
    try:
        project_root = os.path.dirname(BASE_DIR)
        real_ds_path = os.path.join(project_root, "data", "real_sites_dataset.json")
        if os.path.exists(real_ds_path):
            with open(real_ds_path, 'r', encoding='utf-8') as f:
                master_ds = json.load(f)
            master_sites = master_ds.setdefault("sites", {})
            master_tiers = master_ds.setdefault("tier_index", {})
            master_sites[site_id] = {
                ...
                'is_custom': True,
                'polygon': [{'x': round(p[0], 2), 'y': round(p[1], 2)} for p in centered_site_verts],
                'buildings_count': len(shifted_bldgs),
                'target_far': metrics['floor_area_ratio'],
                'metrics': metrics,
                'radius': 100.0,
            }
            if site_id not in master_tiers.setdefault(area_tier, []):
                master_tiers[area_tier].append(site_id)
            master_ds['total_sites'] = len(master_sites)
            with open(real_ds_path, 'w', encoding='utf-8') as f:
                json.dump(master_ds, f)
    except Exception as e:
        print(f"[CustomFetcher] Warning: failed to update real_sites_dataset: {e}")
