...
def fetch_city_osm(lat, lon, dist_m=400):
    query = f"""
    [out:json][timeout:25];
    (
    ...
    for idx, elem in enumerate(data['elements']):
        if elem['type'] == 'way' and 'nodes' in elem:
            ...
            try:
                ...
                if not poly.is_valid or poly.area < 20.0:
                    continue
                area_m2 = poly.area
...
def harvest_real_portfolio():
    ...
    for city_key, info in TARGET_CITIES.items():
        ...
        osm_data = fetch_city_osm(ref_lat, ref_lon, dist_m=400)
        parsed_bldgs = parse_osm_polygons(osm_data, ref_lat, ref_lon, default_h=default_h)

        if not parsed_bldgs:
            print(f"  [Warning] Could not parse building features for {city_key}. Using city fallback.")
            continue

        print(f"  [Success] Parsed {len(parsed_bldgs)} real OSM building footprints for {city_code.upper()}!")

        # Candidate site selection (sites with 150m² <= area <= 5000m²)
        candidate_sites = [b for b in parsed_bldgs if 150.0 <= b['area_m2'] <= 5000.0]
        np.random.seed(42)
        ...
        time.sleep(1.0) # Polite pause between city requests
    ...
    print(f"\n[Complete] Successfully harvested {len(master_dataset)} 100% REAL OpenStreetMap urban context sites!")
