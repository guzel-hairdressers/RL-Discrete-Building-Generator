...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  #controls-bar {{
    ...
    gap: 6px; background: rgba(255, 255, 255, 0.95);
  ...
  #gizmo-container {{
    position: absolute; top: 58px; right: calc(25vw + 18px); z-index: 100;
...
window.addEventListener('message', (event) => {{
  if (!event.data || event.data.type !== 'optimizer_placements') return;
  ...
  const roomLitColor = new THREE.Color(theme.room || '#ccccff');
  const roomShadowColor = new THREE.Color(theme.roomShadow || '#9999ff');
  const edgeHex = theme.edge ? (typeof theme.edge === 'string' ? parseInt(theme.edge.replace('#', '0x')) : theme.edge) : 0x000000;
  const edgeMat = new THREE.LineBasicMaterial({{ color: edgeHex, linewidth: 2.5 }});
...
shadowPlane.position.y = 0.28; // Placed above ground and parcel so shadows project crisply
...
function createArchitecturalVolume(verts, faces, colorHex = 0xffffff, opacity = 1.0, isRoad = false) {{
  ...
  const selfShadowColor = new THREE.Color(0xffffff); // 100% Pure Paper White (0% Grey)
