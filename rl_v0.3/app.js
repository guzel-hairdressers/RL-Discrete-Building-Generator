(() => {
  ...
  function updateDictionaryUI() {
    ...
    const dictionary = Array.isArray(state.dictionary) ? state.dictionary : [];
    ...
    for (const module of dictionary) {
      ...
      swatch.textContent = String(module.id ?? '?').slice(0, 4);
      ...
      detail.textContent = `${category}${Number.isFinite(area) ? ` · ${formatArea(area)}` : ''}`;
  ...
  function drawScaleBar(ctx) {
    ...
    ctx.save();
    ctx.strokeStyle = 'rgba(17,23,18,0.82)';
    ctx.fillStyle = '#364038';
    ctx.lineWidth = 1.25;
    ctx.lineCap = 'square';
    ...
    ctx.beginPath();
    ctx.moveTo(x + offsets[0], y);
    ctx.lineTo(x + offsets[1], y);
    ctx.lineTo(x + offsets[1], y - 4);
    ctx.lineTo(x + offsets[2], y - 4);
    ctx.lineTo(x + offsets[2], y);
    ctx.lineTo(x + offsets[3], y);
    ctx.lineTo(x + offsets[3], y - 4);
    ctx.lineTo(x + offsets[4], y - 4);
    ctx.lineTo(x + offsets[4], y);
    ctx.stroke();
    ...
    offsets.forEach((offset, index) => {
      ...
      ctx.fillText(`${formatDistance(factor * distance)}m`, x + offset, y - 10);
    });
    ctx.restore();
  }
  ...
  function scheduleWallCache(onComplete) {
    ...
    const snapshot = state.placementOrder
      .map((key) => state.placements.get(key))
      .filter(Boolean)
      .map((placement) => ({
        key: placement.key,
