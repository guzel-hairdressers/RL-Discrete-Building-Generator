"""Deterministic vector geometry for Module Lab v0.3.
...
def _connection_ready_source_polygon(
    ...
    points.extend((_point(top_offset + top_width, height), _point(top_offset, height)))
...
def _canonicalize_module_polygon(poly: Sequence[dict]) -> tuple[list[dict], int, float, float, int | None]:
    ...
    for length, index in supporting_edges:
        ...
        for other_length, other_index in supporting_edges:
            ...
            if cross <= COLLINEAR_EPSILON * length * other_length and dot < -EPSILON:
                ...
                break
    if len(canonical_source) > 3 and not parallel_pairs:
        raise RuntimeError("non-triangle modules require opposite parallel connection walls")
    if parallel_pairs:
    ...
    return canonical, connection_index, connection_length, rotation_degrees, opposite_index
...
def _module_record(
    ...
    poly, connection_index, connection_length, canonical_rotation, opposite_index = _canonicalize_module_polygon(poly)
...
def _normalize_latent_vector(latent: Sequence[float], expanded_size: int = 8) -> tuple[list[float], list[float], int]:
    """Validate [0,1] latent controls and deterministically expand short vectors."""

    values = [float(value) for value in latent]
    if not values:
        raise ValueError("latent vector must contain at least one normalized value")
    if any(not math.isfinite(value) or value < 0.0 or value > 1.0 for value in values):
        raise ValueError("latent values must be finite and lie in [0, 1]")
    seed = 0x811C9DC5
    for index, value in enumerate(values):
        quantized = int(round(value * 1_000_000_000.0)) & 0xFFFFFFFF
        seed ^= (quantized + index * 0x9E3779B9) & 0xFFFFFFFF
        seed = RNG._imul(seed, 0x01000193)
        seed ^= seed >> 16
    expanded = list(values[:expanded_size])
    expansion_rng = RNG(seed ^ 0xA5C31E27)
    while len(expanded) < expanded_size:
        expanded.append(expansion_rng.next_val())
    return values, expanded, seed & 0xFFFFFFFF


def _latent_corridor_quadrilateral(
    min_edge: float,
    max_edge: float,
    controls: Sequence[float],
) -> tuple[list[dict], dict]:
    """Map latent controls directly to a legal narrow four-edge corridor."""

    physical_side_limit = 1.5 / math.sin(math.radians(40.0))
    if min_edge > physical_side_limit + COLLINEAR_EPSILON:
        raise ValueError("minEdge is incompatible with a <=1.5 m corridor and 40 degree angles")
    length_low = max(2.4, min_edge)
    length_high = min(12.0, max_edge)
    if length_high < length_low - COLLINEAR_EPSILON:
        raise ValueError("maxEdge is too short for a usable corridor")
    length = length_low + (length_high - length_low) * controls[0]

    if min_edge <= 1.5 + COLLINEAR_EPSILON:
        width_low = max(0.8, min_edge)
        if width_low > 1.5 + COLLINEAR_EPSILON:
            raise ValueError("corridor width cannot satisfy minEdge")
        width = width_low + (1.5 - width_low) * controls[2]
        maximum_shear_for_angle = width / math.tan(math.radians(40.0))
        maximum_shear_for_edge = math.sqrt(max(0.0, max_edge * max_edge - width * width))
        maximum_shear = min(maximum_shear_for_angle, maximum_shear_for_edge)
        shear = (controls[4] * 2.0 - 1.0) * maximum_shear * 0.92
        side_length = math.hypot(width, shear)
        side_angle = math.degrees(math.atan2(width, abs(shear))) if abs(shear) > EPSILON else 90.0
    else:
        side_high = min(max_edge, physical_side_limit)
        if side_high < min_edge - COLLINEAR_EPSILON:
            raise ValueError("edge bounds cannot form a narrow corridor")
        side_length = min_edge + (side_high - min_edge) * controls[2]
        maximum_angle = math.degrees(math.asin(clamp(1.5 / side_length, -1.0, 1.0)))
        side_angle = 40.0 + (maximum_angle - 40.0) * controls[3]
        width = side_length * math.sin(math.radians(side_angle))
        direction = -1.0 if controls[4] < 0.5 else 1.0
        shear = direction * side_length * math.cos(math.radians(side_angle))
    ...
    if not _valid_generated_module_polygon(poly, min_edge, max_edge, True):
        raise ValueError("latent corridor controls cannot satisfy the active edge bounds")
    if min_polygon_width(poly) > 1.5 + COLLINEAR_EPSILON:
        raise ValueError("latent corridor exceeds the 1.5 m true-width limit")
    return poly, {
...
def synthesize_module_from_latent(
    ...
    """Synthesize one constrained module record from normalized learned controls.

    The public bridge is intentionally bounded and side-effect free.  Latent
    dimensions map to target area, side count, aspect, concavity, shear/inset
    proposal, and deterministic fine-shape variation.  Short latent vectors are
    deterministically expanded; supplied values must always lie in [0, 1].

    Raises ``ValueError`` when the requested category or cross-setting constraints
    are physically incompatible.  It never returns an edge-range-relaxed learned
    candidate.
    """
    ...
    if category == "special" and not bool(settings.get("publicMode", False)):
        raise ValueError("special modules require publicMode")
    if not str(identifier):
        raise ValueError("identifier must be nonempty")

    original_latent, controls, latent_seed = _normalize_latent_vector(latent)
    min_edge = max(0.25, float(settings.get("minEdge", 1.5)))
    max_edge = max(min_edge, float(settings.get("maxEdge", 9.0)))
    max_sides = int(clamp(int(settings.get("maxEdges", 8)), 3, 12))
    latent_metadata = {
        "input": original_latent,
        "expanded": controls,
        "seed": latent_seed,
        "areaControl": controls[0],
        "sideCountControl": controls[1],
        "aspectControl": controls[2],
        "concavityControl": controls[3],
        "shearInsetControl": controls[4],
        "variationControl": controls[5],
    }

    if category == "corridor":
        use_triangle = max_sides == 3 or controls[1] < 0.12
        if use_triangle:
            poly, compatible, parameters = _generate_triangle_corridor(
                RNG(latent_seed ^ 0xC0771D0F),
                min_edge,
                max_edge,
            )
            if not compatible:
                raise ValueError("latent triangle corridor cannot satisfy active edge bounds")
        else:
            poly, parameters = _latent_corridor_quadrilateral(min_edge, max_edge, controls)
            compatible = True
        parameters = {**parameters, "latent": latent_metadata, "learnedGeometry": True}
        record = _module_record(
            str(identifier),
            "Learned transit link",
            category,
            poly,
            "learned-narrow-triangle" if len(poly) == 3 else "learned-narrow-link",
            compatible,
            parameters,
        )
    ...
    target_area = area_range[0] + (area_range[1] - area_range[0]) * controls[0]
    requested_sides = 3 + min(max_sides - 3, int(controls[1] * (max_sides - 2)))
    target_aspect = aspect_range[0] + (aspect_range[1] - aspect_range[0]) * controls[2]
    requested_concavity = controls[3] >= 0.5 and max_sides >= 5
    if requested_concavity:
        requested_sides = max(5, requested_sides)

    side_order = [requested_sides] + [
        sides
        for sides in sorted(range(3, max_sides + 1), key=lambda sides: (abs(sides - requested_sides), sides))
        if sides != requested_sides
    ]
    aspect_candidates = [
        target_aspect,
        target_aspect * 0.75 + 0.25,
        target_aspect * 0.5 + 0.5,
        1.0,
    ]
    proposal = 0
    for concave in ((requested_concavity, False) if requested_concavity else (False,)):
        for sides in side_order:
            if concave and sides < 5:
                continue
            for aspect in aspect_candidates:
                proposal_rng = RNG(
                    latent_seed
                    ^ ((proposal + 1) * 0x9E3779B9)
                    ^ int(controls[4] * 0xFFFFFFFF)
                    ^ (int(controls[5] * 0xFFFFFFFF) >> 1)
                )
                proposal += 1
                try:
                    poly, _, compatible, actual_requested_sides, generation_parameters = _generate_area_polygon(
                        proposal_rng,
                        ...
                        force_sides=sides,
                        concave=concave,
                        allow_edge_relaxation=False,
                    )
                except RuntimeError:
                    continue
                if not compatible or len(poly) > max_sides:
                    continue
                actual_concavity = reflex_vertex_count(poly) > 0
                family = f"learned-{'concave-' if actual_concavity else ''}{len(poly)}gon"
                source_parameters = {
                    **generation_parameters,
                    "targetArea": target_area,
                    "targetAspect": target_aspect,
                    "requestedSides": requested_sides,
                    "proposalSides": actual_requested_sides,
                    "requestedConcavity": requested_concavity,
                    "proposalIndex": proposal - 1,
                    "latent": latent_metadata,
                    "learnedGeometry": True,
                }
                record = _module_record(
                    str(identifier),
                    f"Learned {category} module",
                    category,
                    poly,
                    family,
                    True,
                ...
                record["learnedGeometry"] = True
                return record
    raise ValueError("latent controls cannot synthesize a module under the active constraints")
...
def synthesize_module(
    ...
    """Concise alias for :func:`synthesize_module_from_latent`."""

    return synthesize_module_from_latent(settings, category, latent, identifier)


def generate_module_pool(settings: dict, rng: RNG | int | float, count: int = 48) -> list[dict]:
    """Generate a balanced, fully parameterized module candidate pool.

    No fixed polygon template list is used.  Core, room, and public special shapes
    are synthesized from target area, side count, angular, radial, aspect, and
    shear parameters.  Corridors use a parameterized narrow quadrilateral because
    their <=1.5 m true-width invariant is programmatic, not stylistic.

    Cores always occupy 20--30 m².  Special rooms are emitted only in public mode.
    Triangles are explicit metadata-bearing candidates so a policy can penalize
    rather than accidentally treat them as ordinary rooms.
    """

    local_rng = _coerce_rng(rng)
    target_count = max(12, int(count))
    min_edge = max(0.25, float(settings.get("minEdge", 1.5)))
    max_edge = max(min_edge, float(settings.get("maxEdge", 9.0)))
    max_sides = int(clamp(int(settings.get("maxEdges", 8)), 3, 12))
    public_mode = bool(settings.get("publicMode", False))

    # Single-floor mode waives mandatory first-core placement and future stacking;
    # it does not remove cores from the candidate vocabulary or room reachability.
    core_count = max(3, int(round(target_count * 0.15)))
    corridor_count = max(3, int(round(target_count * 0.20)))
    special_count = max(2, int(round(target_count * 0.12))) if public_mode else 0
    minimum_rooms = 4
    allocated = core_count + corridor_count + special_count
    if allocated + minimum_rooms > target_count:
        target_count = allocated + minimum_rooms
    room_count = target_count - allocated

    pool = []
    core_sides = list(range(min(4, max_sides), max_sides + 1))
    core_sides = [sides for sides in core_sides if sides >= 3] or [3]
    room_sides = list(range(3, max_sides + 1))
    special_sides = list(range(min(4, max_sides), max_sides + 1))
    special_sides = [sides for sides in special_sides if sides >= 3] or [3]

    for index in range(core_count):
        request_concave = max_sides >= 5 and index % 3 == 1
        poly, target_area, compatible, requested_sides, generation_parameters = _generate_area_polygon(
            local_rng,
            (20.0, 30.0),
            min_edge,
            max_edge,
            core_sides,
            (0.72, 1.38),
            concave=request_concave,
        )
        concave = reflex_vertex_count(poly) > 0
        pool.append(
            _module_record(
                f"C{index + 1:03d}",
                "Parameterized service core",
                "core",
                poly,
                f"procedural-{'concave-' if concave else ''}{len(poly)}gon",
                compatible,
                {
                    "targetArea": target_area,
                    "requestedSides": requested_sides,
    ...
    for index in range(corridor_count):
        poly, compatible, parameters = _generate_corridor_polygon(
            local_rng,
            min_edge,
            max_edge,
            max_sides,
        ...
        pool.append(
            _module_record(
                f"T{index + 1:03d}",
                "Parameterized transit link",
                "corridor",
                poly,
                "procedural-narrow-triangle" if len(poly) == 3 else "procedural-narrow-link",
                compatible,
                parameters,
            )
        )
    ...
    for index in range(room_count):
        forced_sides = 3 if index % 7 == 0 else None
        request_concave = max_sides >= 5 and forced_sides is None and index % 4 == 2
        poly, target_area, compatible, requested_sides, generation_parameters = _generate_area_polygon(
            local_rng,
            (8.0, 24.5),
            min_edge,
    ...
    if public_mode:
        for index in range(special_count):
            request_concave = max_sides >= 5 and index % 2 == 1
            poly, target_area, compatible, requested_sides, generation_parameters = _generate_area_polygon(
                local_rng,
                (26.0, 46.0),
                min_edge,
    ...
    # Interleave shuffled category queues so even a capped prefix contains a
    # usable program mix instead of exhausting all cores or corridors first.
    category_order = ("core", "corridor", "room", "special")
    queues = {
        category: local_rng.shuffle([module for module in pool if module["category"] == category])
    ...
    while any(queues.values()):
        for category in category_order:
            if queues[category]:
                interleaved.append(queues[category].pop())
    return interleaved
