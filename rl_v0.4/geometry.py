...
def synthesize_module_from_latent(
    ...
    # Triangles only as extreme last resort (< 2% of latent space)
    min_allowed_sides = 4
    requested_sides = min_allowed_sides + min(max_sides - min_allowed_sides, int(controls[1] * (max_sides - min_allowed_sides + 1)))
    ...
    side_order = [requested_sides] + [
        sides
        for sides in sorted(range(min_allowed_sides, max_sides + 1), key=lambda sides: (abs(sides - requested_sides), sides))
...
def generate_module_pool(settings: dict, rng: RNG | int | float, count: int = 48) -> list[dict]:
    ...
    room_sides = list(range(4, max_sides + 1))
    room_sides = [sides for sides in room_sides if sides >= 4] or [4]
