...
class FloorEnvironment:
    ...
    def _validate_edge_alignment(self, candidate_poly: Sequence[dict]) -> bool:
        for i, first in enumerate(candidate_poly):
            ...
            # Check all overlaps on this candidate edge
            for placement in self.placements:
                ...
                for j, third in enumerate(other_poly):
                    ...
                    if interval is not None:
                        ...
                        # Candidate edge overlap fraction must be either ~100% or ~50%
                        is_full = abs(overlap_fraction - 1.0) <= 0.05
                        is_half = abs(overlap_fraction - 0.5) <= 0.05
                        if not (is_full or is_half):
                            return False
                        
                        # If halfway connection, it must be either the left half (start ~0) or right half (end ~1) strictly
                        if is_half and not (start <= 1e-7 or end >= 1.0 - 1e-7):
                            return False
                        
                        # Neighbor edge overlap fraction must also be either ~100% or ~50%
                        ...
                        overlap_fraction_other = end_other - start_other
                        
                        is_full_other = abs(overlap_fraction_other - 1.0) <= 0.05
                        is_half_other = abs(overlap_fraction_other - 0.5) <= 0.05
                        if not (is_full_other or is_half_other):
                            return False
                        
                        # If halfway connection on neighbor, it must be either left or right half strictly
                        if is_half_other and not (start_other <= 1e-7 or end_other >= 1.0 - 1e-7):
                            return False
        return True
...
class ParallelTrainer:
    ...
    def _synthesize_dictionary(
        ...
        for slot_index in range(cap):
            ...
            # Rotation offset disabled — diversity comes from _attachment_indices
            # returning all edges so modules can attach in any direction.
            rotation_offset = 0.0
            ...
            for attempt in range(24):
                ...
                # Inject diversity noise to ensure different slots generate distinct shapes
                raw_action = raw_action + torch.randn_like(raw_action) * 0.22
                attempt_log_probs.append(geometry_distribution.log_prob(raw_action - torch.randn_like(raw_action) * 0.0)) # keep log_prob calculation clean
                ...
                try:
                    proposal = G.synthesize_module_from_latent(
                        settings, category, latent, identifier, rotation_offset
                    )
    ...
    def _aggregate_terminal(self, per_site: Sequence[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Piecewise linear scaling for fill ratio (< 50% penalized heavily)
        scaled_fill = max(0.0, 2.5 * fill_ratio - 0.75) if fill_ratio < 0.5 else fill_ratio

        # Piecewise linear scaling for rentable ratio (< 60% penalized heavily)
        scaled_rentable = max(0.0, 2.5 * rentable_ratio - 0.9) if rentable_ratio < 0.6 else rentable_ratio

        # Area variance penalty of dictionary shapes (minimize variance of size)
        dict_areas = [G.polygon_area(m["poly"]) for m in self.dictionary]
        area_variance_penalty = 0.012 * _std(dict_areas) if dict_areas else 0.0

        # Exposed internal walls penalty
        total_internal_exposed = math.fsum(float(item.get("internalExposedPerimeter", 0.0)) for item in per_site)
        # Normalize internal exposed wall penalty relative to total building perimeter
        internal_exposed_penalty = 0.35 * _safe_ratio(total_internal_exposed, perimeter)
