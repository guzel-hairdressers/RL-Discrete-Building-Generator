...
class ProceduralGeometryTests(unittest.TestCase):
    ...
    def test_every_module_has_canonical_horizontal_connection_edges(self) -> None:
        ...
        core = next(module for module in pool if module["category"] == "core" and len(module["poly"]) > 3)
        room = next(module for module in pool if module["category"] == "room" and len(module["poly"]) > 3)
