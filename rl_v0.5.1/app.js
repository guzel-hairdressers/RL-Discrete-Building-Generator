...
(() => {
  ...
  function handleSiteEvent(data) {
    ...
    if (state.settingsDirty && state.connected) {
      state.generationId = data.generationId;
      state.episode = data.episode ?? 0;
      state.awaitingSite = false;
      applySettingsTransaction('Applying queued settings to all environments');
      return;
    }
    ...
    if (Array.isArray(data.scoreHistory)) {
      state.scoreHistory = data.scoreHistory.map(Number).filter(Number.isFinite).slice(-60);
    }
  ...
  function handleEpisodeDoneEvent(data) {
    ...
    if (Array.isArray(data.scoreHistory)) state.scoreHistory = data.scoreHistory.map(Number).filter(Number.isFinite).slice(-60);
    ...
    scheduleWallCache(() => {
      ...
      state.transitionTimer = window.setTimeout(() => {
        if (state.trainingWanted && state.pendingNextEpisode !== null && !state.awaitingSite) beginNextEpisode();
      }, 650);
    });
  }
  ...
  function addPlacementArea(placement) {
    ...
    if (placement.module.category === 'room' || placement.module.category === 'special') {
      areaState.rentable += placement.area;
      state.totalRentableArea += placement.area;
    }
  ...
  function removePlacementArea(placement) {
    ...
    if (placement.module.category === 'room' || placement.module.category === 'special') {
      areaState.rentable = Math.max(0, areaState.rentable - placement.area);
      state.totalRentableArea = Math.max(0, state.totalRentableArea - placement.area);
    }
  }
  ...
  function updateMetricsUI() {
    ...
    const best = finiteOr(state.bestScore, metricValue(metrics, ['bestScore'], 0));
    ...
    dom.bestScoreValue.textContent = best > 0 ? `Best ${formatDecimal(best, 1)}` : 'No baseline';
  ...
  function updateScoreBreakdown() {
    ...
    addDivider();
    addRow('Raw Layout Score', 'Weighted sum of objectives before penalties', `${rawScore.toFixed(2)} pts`);
    addDivider();
    ...
    addRow('BPE Merge Bonus', `Reused BPE merges (frequency >= 2, +3.0 pts per merge)`, `+${bpeBonus.toFixed(2)} pts`);
    addRow('Unmerged Triangles Penalty', `Count: ${unmergedTriangles} unmerged triangles (-1.5 to -8.0 pts/ea)`, `-${unmergedTrianglePenalty.toFixed(2)} pts`);
    
    addDivider();
    addRow('Total Score', 'Raw Score - Topology Penalty + BPE Bonus - Triangle Penalty', `${totalScore.toFixed(4)} pts`, true);
  }
  ...
  function updateDictionaryUI() {
    ...
    let dictionary = [];
    if (useMerged) {
      const mergedList = Array.isArray(state.mergedDictionary) ? state.mergedDictionary : [];
      // Only display BPE merged shapes that are actually reused (count >= 2) on the current floor
      ...
      // Append any basic shapes that are currently placed (i.e. unmerged)
      const basicList = Array.isArray(state.dictionary) ? state.dictionary : [];
      for (const base of basicList) {
        const count = counts.get(base.id) || 0;
        if (count > 0) {
          // Check if already in dictionary to prevent duplicates
          if (!dictionary.some(m => m.id === base.id)) {
            dictionary.push(base);
          }
        }
      }
    } else {
      dictionary = Array.isArray(state.dictionary) ? state.dictionary : [];
    }
  ...
  function drawHistory() {
    ...
    const values = state.scoreHistory.slice(-40);
    dom.trendValue.textContent = values.length ? formatDecimal(values[values.length - 1], 1) : '—';
    ...
    const points = values.map((value, index) => ({
      x: 9 + (index / (values.length - 1)) * (width - 18),
      y: height - 10 - ((value - min) / span) * (height - 22)
    }));
    ...
    ctx.beginPath();
    ctx.moveTo(points[0].x, height - 8);
