...
class ParallelTrainer:
    ...
    def _synthesize_dictionary(
        ...
        # We only have "core" (0) and "room" (2) categories in training/placement policy
        ...
        for slot_index in range(cap):
            ...
            if not matching_indices:
                matching_indices = list(range(len(basic_shapes)))
    ...
    def _finish_episode(self) -> dict[str, Any]:
        ...
        initial_placements = sum(len(environment.placements) for environment in self.environments)
        vocab_dict = {module.type_id: module for module in merged_vocab}
        total_reused_merges = 0
        from collections import Counter
        for layout_graph in layout_graphs:
            shape_counts = Counter()
            for node in layout_graph.nodes.values():
                shape_counts[node.get("shapeType", "")] += 1
            for shape_type, count in shape_counts.items():
                if shape_type.startswith("M_round") and count >= 2:
                    merged_module = vocab_dict.get(shape_type)
                    if merged_module:
                        num_components = len(merged_module.components)
                        total_reused_merges += count * (num_components - 1)

        if initial_placements > 0 and metrics["topologyValid"]:
            bpe_bonus = 3.0 * (total_reused_merges / max(1, len(layout_graphs)))
        else:
            bpe_bonus = 0.0
            
        # Count unmerged triangles with area-dependent penalty weight (halved to allow learning):
        # - Small (<15 m²): 4.0 points
        # - Medium (15 to 30 m²): 2.0 points
        # - Large (>= 30 m²): 0.75 points
        unmerged_triangles = 0
        unmerged_triangle_penalty_sum = 0.0
        for layout_graph in layout_graphs:
            for node in layout_graph.nodes.values():
                if node.get("triangle", False):
                    unmerged_triangles += 1
                    area = G.polygon_area(node["poly"])
                    if area < 15.0:
                        unmerged_triangle_penalty_sum += 8.0
                    elif area < 30.0:
                        unmerged_triangle_penalty_sum += 4.0
                    else:
                        unmerged_triangle_penalty_sum += 1.5
                    
        # Divide by the number of environments so the penalty is scaled per floor (matching the average score scale)
        unmerged_triangle_penalty = unmerged_triangle_penalty_sum / max(1, len(layout_graphs))
        ...
        return {
            ...
            "placements": merged_placements_formatted,
        }
...
app = FastAPI(title="Module Lab v0.5")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
if __name__ == "__main__":
    print(f"Module Lab v0.5 policy device: {select_device().type}")
