"""Deterministic vector geometry for Module Lab v0.6-A.
...
def generate_basic_dictionary(settings: dict) -> list[dict]:
    ...
    minimum_edge = float(settings.get("minEdge", 1.0))
    ...
    for shape in shapes:
        ...
        edge_lengths = record["parameters"]["edgeLengths"]
        if len(record["poly"]) > max_edges:
            continue
        if any(
            edge_length < minimum_edge - edge_tolerance
            or edge_length > maximum_edge + edge_tolerance
...
def generate_module_pool(settings: dict, rng: RNG | int | float, count: int = 48) -> list[dict]:
    ...
    dictionary = generate_basic_dictionary(settings)
    if not dictionary:
        raise ValueError("active edge constraints leave no feasible basic modules")
    pool = []
