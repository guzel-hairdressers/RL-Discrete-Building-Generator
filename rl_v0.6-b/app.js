...
(() => {
  ...
  function updatePerformanceTimings() {
    ...
    function humanLabel(key) {
      ...
      return humanLabels[key] || key.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase()).trim();
    }
    ...
    function addRow(key, data, isTotal) {
      ...
      const isSub = key.startsWith('cg');
      let labelColor = isTotal ? '#c4db8b' : '#adb9b0';
      if (isSub) labelColor = '#6d7970';
      tdLabel.style.cssText = 'text-align:left;padding:3px 4px;color:' + labelColor + ';white-space:pre;';
      tdLabel.textContent = humanLabel(key);
      tr.appendChild(tdLabel);
      for (const val of [data.avg, data.min, data.max]) {
        const td = document.createElement('td');
        td.style.cssText = 'text-align:right;padding:3px 6px;color:' + (isTotal ? '#c4db8b' : '#dde3dd') + ';';
