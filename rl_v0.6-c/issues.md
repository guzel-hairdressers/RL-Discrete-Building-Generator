# Module Lab v0.6-C - Inherited Issues History
...
> The entries below are the archived v0.5.1 investigation record, retained for traceability. Their historical status labels are not the current v0.6-C status. See `README.md`, `core_stacking_guide.md`, the root `rl_optimization_specs.md`, and the collected tests for authoritative branch behavior. Path C adds atomic exact core stacking, boundary/atrium adaptation, global `+3`-per-reused-module scoring, and canonical single-floor behavior; Path A separately contains the full BPE stabilization work.
