"""Module Lab v0.6-C PyTorch training and WebSocket server.
...
# Four metres around the largest core envelope leaves a legal first room
# attachment frontier instead of merely fitting an isolated structural object.
CORE_RESERVE_PADDING = 4.0
...
class CoreStackingError(RuntimeError):
    """Raised before mutation when a shared core action is no longer legal."""
...
def validate_settings_patch(current: dict[str, Any], patch: Any) -> dict[str, Any]:
    ...
    _validate_fixed_dictionary_seed(merged)
    return merged
...
class CoreStackCandidate:
    """One module transform proven legal on every floor in local coordinates."""
    ...
    def signature(self) -> tuple[str, float, float, float]:
        return (
            str(self.module["id"]),
            round(float(self.rotation.get("angle", 0.0)), 7),
            round(float(self.anchor_x), 7),
            round(float(self.anchor_y), 7),
        )
...
def _mean_feature_rows(rows: Sequence[Sequence[float]]) -> list[float]:
    """Pool equal-length placement features without weighting large floors more."""

    if not rows:
        raise ValueError("cannot pool an empty placement feature set")
    if any(len(row) != PLACEMENT_FEATURE_DIM for row in rows):
        raise ValueError("placement feature dimension mismatch")
    return [
        math.fsum(float(row[index]) for row in rows) / len(rows)
        for index in range(PLACEMENT_FEATURE_DIM)
    ]
...
def _unmerged_triangle_metrics(layout_graphs: Sequence[Any]) -> tuple[int, float]:
    """Return total post-BPE triangles and the canonical per-floor penalty."""

    triangle_count = len(graph.count_post_merge_triangles(layout_graphs))
    average_count = triangle_count / max(1, len(layout_graphs))
    return (
        triangle_count,
        UNMERGED_TRIANGLE_PENALTY_PER_FLOOR * average_count,
    )
...
def _modules_have_compatible_edge(first: dict, second: dict) -> bool:
    """Match the environment's legal 1:1, 1:2, or 2:1 wall ratios."""

    first_poly = first["poly"]
    second_poly = second["poly"]
    first_lengths = [
        math.hypot(
            first_poly[(index + 1) % len(first_poly)]["x"] - point["x"],
            first_poly[(index + 1) % len(first_poly)]["y"] - point["y"],
        )
        for index, point in enumerate(first_poly)
    ]
    second_lengths = [
        math.hypot(
            second_poly[(index + 1) % len(second_poly)]["x"] - point["x"],
            second_poly[(index + 1) % len(second_poly)]["y"] - point["y"],
        )
        for index, point in enumerate(second_poly)
    ]
    return any(
        abs(first_length / second_length - ratio) < 5.0e-3
        for first_length in first_lengths
        for second_length in second_lengths
        if second_length > G.EPSILON
        for ratio in (0.5, 1.0, 2.0)
    )


def _validate_fixed_dictionary_seed(settings: dict[str, Any]) -> None:
    """Reject settings whose filtered fixed vocabulary cannot start a layout."""
    ...
    if bool(settings["singleFloor"]):
        if not rooms:
            raise SettingsError(
                "active edge and vertex limits leave no legal fixed Room module"
            )
        return

    cores = [module for module in modules if module["category"] == "core"]
    if not cores or not rooms or not any(
        _modules_have_compatible_edge(core, room)
        for core in cores
        for room in rooms
    ):
        raise SettingsError(
            "active edge and vertex limits cannot form the required multi-floor Core/Room seed"
        )
...
class FloorEnvironment:
    ...
    def world_boundary(self) -> dict:
        ...
        stacking = self.boundary.get("stacking")
        if isinstance(stacking, dict):
            record["stacking"] = copy.deepcopy(stacking)
            reserve = stacking.get("reserve")
            if isinstance(reserve, list):
                record["stacking"]["worldReserve"] = G.translate_polygon(reserve, dx, dy)
            placement_anchor = stacking.get("placementAnchor")
            if isinstance(placement_anchor, dict):
                record["stacking"]["worldPlacementAnchor"] = {
                    "x": float(placement_anchor["x"]) + dx,
                    "y": float(placement_anchor["y"]) + dy,
                }
        return record
    ...
    def generate_candidates(
        ...
        if allowed_categories is not None:
            modules = [
                module for module in modules if module["category"] in allowed_categories
            ]
        ...
        if not placing_first and not early_break and allowed_categories is None:
        ...
        *,
        ...
        if core_stack_id is not None:
            if placement["category"] != "core":
                raise CoreStackingError("only core modules can be committed as a stack")
            placement.update(
                {
            ...
            public_record.update(
                {
                    ...
                    "localPoly": copy.deepcopy(candidate.poly),
                }
            )
                        ...
                        if self.site.get("holes"):
                            for hole in self.site["holes"]:
                                for k in range(len(hole)):
                                    q1 = hole[k]
                                    q2 = hole[(k + 1) % len(hole)]
                                    if G._segment_intersection_kind(
                                        first["center"], second["center"], q1, q2
                                    ) == "proper":
                                        separated = True
                                        break
                                if separated:
...
class ParallelTrainer:
    ...
    def __init__(self, settings: dict[str, Any] | None = None) -> None:
        ...
        self.last_loss = 0.0
        self.stacking_metadata: dict[str, Any] = {
            "enabled": not bool(self.settings["singleFloor"]),
            "status": "unprepared",
        }
    ...
    @staticmethod
    def _core_reserve_size(settings: dict[str, Any]) -> float:
        """Return a square that contains every fixed core at every 2D rotation."""

        cores = [
            module
            for module in G.generate_basic_dictionary(settings)
            if module["category"] == "core"
        ]
        if not cores:
            raise CoreStackingError("the dictionary source contains no core module")
        ...
        required = maximum_diameter + 2.0 * CORE_RESERVE_PADDING
        return math.ceil(required * 2.0 - 1.0e-9) / 2.0

    def _build_sites(
        self,
        settings: dict[str, Any],
        generation_id: int,
    ) -> tuple[list[FloorEnvironment], list[torch.Tensor], dict[str, Any]]:
        """Build floors around a guaranteed common local core reserve."""
        ...
        floor_sources: list[tuple[dict, G.RNG]] = []
        for index in range(int(settings["parallelEnvironments"])):
            ...
            floor_sources.append((boundary, rng))

        stacking_enabled = not bool(settings["singleFloor"])
        if stacking_enabled:
            reserve_size = self._core_reserve_size(settings)
            try:
                boundaries, reserve_metadata = G.adapt_boundaries_for_core_reserve(
                    [boundary for boundary, _ in floor_sources],
                    reserve_size,
                    reserve_size,
                )
            except ValueError as error:
                raise CoreStackingError(str(error)) from error
            placement_anchor = {
                "x": float(reserve_metadata["anchor"]["x"] + CORE_RESERVE_PADDING),
                "y": float(reserve_metadata["anchor"]["y"] + CORE_RESERVE_PADDING),
            }
            stacking_metadata: dict[str, Any] = {
                "enabled": True,
                "status": "ready",
                "mode": reserve_metadata["mode"],
                "reserve": copy.deepcopy(reserve_metadata["poly"]),
                "reserveAnchor": copy.deepcopy(reserve_metadata["anchor"]),
                "reserveSize": reserve_size,
                "placementAnchor": placement_anchor,
                "adaptedBoundaryIndices": list(
                    reserve_metadata["adaptedBoundaryIndices"]
                ),
                "rejectedAtriumCandidates": {},
                "failureSafe": reserve_metadata["failureSafe"],
            }
        else:
            boundaries = [copy.deepcopy(boundary) for boundary, _ in floor_sources]
            stacking_metadata = {
                "enabled": False,
                "status": "disabled-single-floor-mode",
                "mode": "disabled",
                "adaptedBoundaryIndices": [],
                "rejectedAtriumCandidates": {},
                "failureSafe": "not-required",
            }

        for index, ((_, rng), boundary) in enumerate(zip(floor_sources, boundaries)):
            candidates = G.atrium_candidates(boundary, rng.fork(23))
            rejected_atria: list[str] = []
            if stacking_enabled:
                candidates, rejected_atria = G.atrium_candidates_clear_of_reserve(
                    boundary,
                    candidates,
                    stacking_metadata["reserve"],
                )
                stacking_metadata["rejectedAtriumCandidates"][str(index)] = rejected_atria
            ...
            if stacking_enabled:
                if not G.polygon_inside_site(
                    stacking_metadata["reserve"], site["outer"], site["holes"]
                ):
                    raise CoreStackingError(
                        f"floor {index} invalidated the shared core reserve; generation was not committed"
                    )
                boundary["stacking"] = {
                    "enabled": True,
                    "status": "ready",
                    "mode": stacking_metadata["mode"],
                    "boundaryAdapted": index
                    in stacking_metadata["adaptedBoundaryIndices"],
                    "reserve": copy.deepcopy(stacking_metadata["reserve"]),
                    "placementAnchor": copy.deepcopy(
                        stacking_metadata["placementAnchor"]
                    ),
                    "rejectedAtriumCandidates": list(rejected_atria),
                }
    ...
    def _synthesize_dictionary(
        ...
        # Multi-floor buildings learn Core/Room vocabularies.  In canonical
        # single-floor mode structural stacking classifications are disabled,
        # so every dictionary slot is an ordinary Room action.
        single_floor = bool(settings["singleFloor"])
        ...
        core_count = 0 if single_floor else 1  # Multi-floor slot 0 is Core.
        used_ids = set()
        primary_core_shape: dict | None = None
        ...
        for slot_index in range(cap):
            ...
            def is_stack_growth_compatible(shape: dict) -> bool:
                if bool(settings["singleFloor"]):
                    return True
                if slot_index == 0 and category == "core":
                    return any(
                        candidate["category"] == "room"
                        and _modules_have_compatible_edge(shape, candidate)
                        for candidate in basic_shapes
                    )
                if slot_index == 1 and primary_core_shape is not None:
                    return _modules_have_compatible_edge(primary_core_shape, shape)
                return True
            ...
            matching_indices = [
                ...
                and is_stack_growth_compatible(s)
            ]
            if not matching_indices:
                # Fallback if all shapes of this category have been used
                matching_indices = [
                    i for i, s in enumerate(basic_shapes)
                    if s["category"] == category and is_stack_growth_compatible(s)
                ]
            if not matching_indices:
                raise CoreStackingError(
                    f"dictionary slot {slot_index} has no stack-growth-compatible {category} shape"
                )
            ...
            if slot_index == 0 and not single_floor:
                primary_core_shape = shape_to_use
    ...
    def _core_stack_at_transform(
        ...
        """Return a stack only after the same transform passes every floor."""

        if module.get("category") != "core" or not environments:
            return None
        maximum_modules = int(settings["maxModules"])
        floor_candidates: list[PlacementCandidate] = []
        for environment in environments:
            if len(environment.placements) >= maximum_modules:
                return None
            room_core_costs = (
            ...
            if candidate is None:
                return None
            if environment.placements and not environment._validate_edge_alignment(
                candidate.poly
            ):
                return None
            floor_candidates.append(candidate)

        reference_poly = floor_candidates[0].poly
        reference_angle = float(floor_candidates[0].rotation.get("angle", 0.0))
        if any(candidate.poly != reference_poly for candidate in floor_candidates[1:]):
            raise CoreStackingError("shared core validation produced unequal local polygons")
        if any(
            float(candidate.rotation.get("angle", 0.0)) != reference_angle
            for candidate in floor_candidates[1:]
        ):
            raise CoreStackingError("shared core validation produced unequal rotations")
    ...
    @classmethod
    def _initial_core_stack_candidates_for(
        cls,
        environments: Sequence[FloorEnvironment],
        ...
        limit: int = CORE_STACK_CANDIDATE_LIMIT,
    ) -> list[CoreStackCandidate]:
        if not stacking_metadata.get("enabled"):
            return []
        anchor = stacking_metadata.get("placementAnchor")
        if not isinstance(anchor, dict):
            raise CoreStackingError("shared core reserve has no placement anchor")
        stacks: list[CoreStackCandidate] = []
        for module in dictionary:
            if module.get("category") != "core":
                continue
            for rotation in module.get("rotations", []):
                stack = cls._core_stack_at_transform(
                    environments,
                    module,
                    rotation,
                    float(anchor["x"]),
    ...
    def _shared_core_stack_candidates(
        ...
        """Collect shared core actions; never expose independent core actions."""

        if bool(self.settings["singleFloor"]) or not self.environments:
            return []
        if any(
            len(environment.placements) >= int(self.settings["maxModules"])
            for environment in self.environments
        ):
            return []
        if not any(environment.core_ids for environment in self.environments):
            return self._initial_core_stack_candidates_for(
                self.environments,
                self.dictionary,
                self.settings,
                self.stacking_metadata,
                orientation_basis,
                limit,
            )

        specifications: dict[tuple[str, float, float, float], tuple[dict, dict, float, float]] = {}
        source_limit = max(8, limit)
        for environment in self.environments:
            candidates = environment.generate_candidates(
                self.settings,
                orientation_basis,
                limit=source_limit,
                allowed_categories={"core"},
            )
            for candidate in candidates:
                signature = (
                    str(candidate.module["id"]),
                    round(float(candidate.rotation.get("angle", 0.0)), 7),
                    round(float(candidate.anchor_x), 7),
                    round(float(candidate.anchor_y), 7),
                )
        ...
        stacks: list[CoreStackCandidate] = []
        for signature in sorted(specifications):
            module, rotation, anchor_x, anchor_y = specifications[signature]
            stack = self._core_stack_at_transform(
                self.environments,
                module,
                rotation,
                anchor_x,
    ...
    def _commit_core_stack(
        self,
        stack: CoreStackCandidate,
        trigger_floor: int | None,
        policy_log_prob: torch.Tensor | None = None,
    ) -> tuple[list[dict], dict[str, Any]]:
        """Atomically commit one shared core transform to every floor."""

        if bool(self.settings["singleFloor"]):
            raise CoreStackingError("core stacking is disabled in single-floor mode")
        if trigger_floor is not None and (
            trigger_floor < 0 or trigger_floor >= len(self.environments)
        ):
            raise CoreStackingError("core stack trigger floor is out of range")
        refreshed = self._core_stack_at_transform(
            self.environments,
            stack.module,
            stack.rotation,
            stack.anchor_x,
        ...
        if refreshed is None:
            raise CoreStackingError(
                "shared core action became illegal on at least one floor; no floor was mutated"
            )

        stack_id = (
            f"g{self.generation_id}:e{self.episode}:core-stack-{len(self.core_stack_records)}"
        )
        snapshots = [copy.deepcopy(environment.__dict__) for environment in self.environments]
        placements: list[dict] = []
        try:
            for environment, candidate in zip(
                self.environments, refreshed.floor_candidates
            ):
                placements.append(
                    environment.place(
                        candidate,
                        core_stack_id=stack_id,
                        core_stack_trigger_floor=trigger_floor,
                    )
                )
        except Exception as error:
            for environment, snapshot in zip(self.environments, snapshots):
                environment.__dict__.clear()
                environment.__dict__.update(snapshot)
            raise CoreStackingError(
                "core stack commit failed and every floor was rolled back"
            ) from error

        if policy_log_prob is not None:
            # One physical stack is one policy action, regardless of floor count.
            self.placement_log_probs.append(policy_log_prob)
        for environment in self.environments:
            environment.done = len(environment.placements) >= int(
                self.settings["maxModules"]
            )
        record = {
            "id": stack_id,
            "moduleId": str(stack.module["id"]),
            "rotation": float(stack.rotation.get("angle", 0.0)),
            "localAnchor": {
                "x": float(stack.anchor_x),
                "y": float(stack.anchor_y),
            },
            "localPoly": copy.deepcopy(refreshed.floor_candidates[0].poly),
            "triggerFloor": int(trigger_floor) if trigger_floor is not None else None,
            "decisionScope": "building" if trigger_floor is None else "floor-propagated",
            "floorCount": len(placements),
            "placementIds": [placement["id"] for placement in placements],
            "locked": True,
            "exactLocalAlignment": True,
        }
        self.core_stack_records.append(record)
        return placements, record

    def _stacking_event(self) -> dict[str, Any]:
        event = copy.deepcopy(self.stacking_metadata)
        violations: list[str] = []
        for record in self.core_stack_records:
            ...
            placements = [
                next(
                    (
                        ...
                        if placement.get("coreStackId") == stack_id
                    ),
                    None,
                )
                for environment in self.environments
            ...
            if any(placement is None for placement in placements):
                violations.append(f"{stack_id}:missingFloor")
                continue
            concrete = [placement for placement in placements if placement is not None]
            reference = concrete[0]
            if any(placement.get("moduleId") != reference.get("moduleId") for placement in concrete[1:]):
                violations.append(f"{stack_id}:moduleMismatch")
            if any(placement.get("rotation") != reference.get("rotation") for placement in concrete[1:]):
                violations.append(f"{stack_id}:rotationMismatch")
            if any(
                placement.get("localAnchor") != reference.get("localAnchor")
                for placement in concrete[1:]
            ):
                violations.append(f"{stack_id}:anchorMismatch")
            if any(placement.get("poly") != reference.get("poly") for placement in concrete[1:]):
                violations.append(f"{stack_id}:localPolygonMismatch")
            if any(not placement.get("coreStackLocked") for placement in concrete):
                violations.append(f"{stack_id}:unlocked")
        if event.get("enabled"):
            unstacked = [
                ...
                and not placement.get("coreStackLocked")
            ]
            if unstacked:
                violations.append("unstackedCore")
        event.update(
            {
                ...
                "stacks": copy.deepcopy(self.core_stack_records),
    ...
    def _stacking_metrics(self) -> dict[str, Any]:
        event = self._stacking_event()
        return {
            "enabled": bool(event.get("enabled")),
            "status": str(event.get("status", "unknown")),
            "mode": str(event.get("mode", "unknown")),
            "stackCount": int(event["stackCount"]),
            "lockedCoreCount": int(event["lockedCoreCount"]),
            "exactLocalAlignment": bool(event["exactLocalAlignment"]),
            "adaptedBoundaryCount": len(event.get("adaptedBoundaryIndices", [])),
            "rejectedAtriumCandidateCount": sum(
                len(items)
                for items in event.get("rejectedAtriumCandidates", {}).values()
            ),
        }

    def _initial_stacks_on_empty_floors(
        self,
        dictionary: Sequence[dict],
        stacking_metadata: dict[str, Any],
        orientation_basis: float = 0.0,
    ) -> list[CoreStackCandidate]:
        """Validate a prospective episode without clearing the completed one."""
        ...
        empty_environments = [
            FloorEnvironment(
                ...
                environment.site,
                environment.offset,
                G.RNG(environment.rng.state),
            )
            for environment in self.environments
        ]
        for environment in empty_environments:
    ...
    def _preflight_next_episode_stacking(self, next_episode: int) -> dict[str, int]:
        """Prove every policy-selectable primary Core works before learning/reset.

        The persistent reserve was sized for every fixed Core, but this explicit
        preflight makes the failure path transactional too: a failed next-episode
        stack check occurs before policy learning, counters, dictionaries, or
        live floor states are changed.
        """

        if not self.stacking_metadata.get("enabled"):
            return {}
        basic_shapes = G.generate_basic_dictionary(self.settings)
        rooms = [shape for shape in basic_shapes if shape["category"] == "room"]
        cores = [
            shape
            for shape in basic_shapes
            if shape["category"] == "core"
            and any(_modules_have_compatible_edge(shape, room) for room in rooms)
        ]
        if not cores:
            raise CoreStackingError(
                "the next episode has no stack-growth-compatible primary Core"
            )
        candidate_counts: dict[str, int] = {}
        for source in cores:
            module = copy.deepcopy(source)
            module["id"] = "s0"
            module = self._canonical_module(
                module,
                float(self.settings["angleStep"]),
                phase=next_episode,
            )
            initial_stacks = self._initial_stacks_on_empty_floors(
                [module], self.stacking_metadata
            )
            if not initial_stacks:
                raise CoreStackingError(
                    "the persistent site reserve cannot accept every next-episode Core; "
                    "the completed episode was retained"
                )
            candidate_counts[str(source["id"])] = len(initial_stacks)
    ...
    def _prepare_generation(
        ...
        generation_id: int,
        episode: int,
    ) -> tuple[list[FloorEnvironment], list[dict], list[torch.Tensor], dict[str, Any]]:
        environments, atrium_logs, stacking_metadata = self._build_sites(
        ...
        for environment in environments:
            environment.reset(dictionary)
        if stacking_metadata.get("enabled"):
            stacking_metadata["growthSeed"] = {
                "coreModuleId": dictionary[0]["id"],
                "roomModuleId": dictionary[1]["id"],
                "compatibleEdgeRatio": True,
            }
            initial_stacks = self._initial_core_stack_candidates_for(
                environments,
                dictionary,
                settings,
                stacking_metadata,
            ...
            if not initial_stacks:
                raise CoreStackingError(
                    "the adapted sites have no legal shared core action; generation was not committed"
                )
            stacking_metadata["initialCandidateCount"] = len(initial_stacks)
        else:
            stacking_metadata["initialCandidateCount"] = 0
        return environments, dictionary, atrium_logs + shape_logs, stacking_metadata
    ...
    def _commit_generation(
        ...
        self.stacking_metadata = copy.deepcopy(stacking_metadata)
    ...
    def update_settings(self, patch: Any) -> dict[str, Any]:
        ...
        environments, dictionary, shape_logs, stacking_metadata = self._prepare_generation(
            proposed, generation, self.episode
        )
    ...
    def _aggregate_online(self) -> dict[str, Any]:
        ...
        if self.stacking_metadata.get("enabled"):
            metrics["coreStacking"] = self._stacking_metrics()
        return metrics
    ...
    def _aggregate_terminal(self, per_site: Sequence[dict[str, Any]]) -> dict[str, Any]:
        ...
        stacking_event = self._stacking_event()
        stacking_invalid = bool(stacking_event.get("enabled")) and not bool(
            stacking_event.get("exactLocalAlignment")
        )
        violation_rate = _clamp(
            _safe_ratio(len(invalid_sites), len(per_site))
            + 0.08 * _safe_ratio(violation_count, max(1, len(per_site)))
            + (1.0 if stacking_invalid else 0.0)
        )
        ...
        violations.extend(
            f"building:{violation}"
            for violation in stacking_event.get("violations", [])
        )
        metrics = {
            "filledArea": filled,
            "siteArea": site_area,
            ...
            "topologyValid": not invalid_sites and not stacking_invalid,
            "topologyViolations": violations,
    ...
    def _finish_episode(self) -> dict[str, Any]:
        ...
        next_episode = completed_episode + 1
        # Fail before terminal learning or any episode/reset mutation if the
        # persistent sites cannot support the complete next Core source set.
        preflight_candidate_counts = self._preflight_next_episode_stacking(
            next_episode
        )
        completed_dictionary = [
            _public_module(module) for module in self.dictionary
        ]
        completed_stacking = self._stacking_event()
        per_site = [
        ...
        # Count post-merge polygons geometrically, not from stale placement flags.
        # The canonical v0.5.1 reward is exactly eight points per unmerged
        # triangle, averaged across floors to match the aggregate score scale.
        unmerged_triangles, unmerged_triangle_penalty = (
            _unmerged_triangle_metrics(layout_graphs)
        )
        ...
        # Preserve the completed episode's individual placement records before
        # the environments are reset.  The client needs both representations
        # to switch between merged and unmerged views without losing geometry.
        ...
        next_dictionary, next_shape_logs = self._synthesize_dictionary(
            self.settings, self.environments, self.generation_id, next_episode
        )
        next_stacking_metadata = copy.deepcopy(self.stacking_metadata)
        if next_stacking_metadata.get("enabled"):
            next_stacking_metadata["growthSeed"] = {
                "coreModuleId": next_dictionary[0]["id"],
                "roomModuleId": next_dictionary[1]["id"],
                "compatibleEdgeRatio": True,
            }
            primary_shape_type = str(
                next_dictionary[0].get(
                    "shapeType",
                    next_dictionary[0].get("parameters", {}).get(
                        "shapeType", ""
                    ),
                )
            )
            initial_candidate_count = preflight_candidate_counts.get(
                primary_shape_type, 0
            )
            if initial_candidate_count <= 0:
                raise CoreStackingError(
                    "the synthesized next-episode Core was absent from the preflight set"
                )
            next_stacking_metadata["initialCandidateCount"] = initial_candidate_count

        # Commit the already prepared next episode only after every shared Core
        # check passed on empty copies of all floors.
        self.episode = next_episode
        self.dictionary = next_dictionary
        self.shape_log_probs = next_shape_logs
        self.placement_log_probs = []
        self.step_number = 0
        self.stacking_metadata = next_stacking_metadata
        for environment in self.environments:
        ...
        return {
            ...
            "dictionary": completed_dictionary,
            "nextDictionary": [
    ...
    def step(self, generation_id: Any, episode: Any) -> dict[str, Any]:
        ...
        if bool(self.settings["singleFloor"]) and all(
            environment.done for environment in self.environments
        ):
        ...
        single_floor = bool(self.settings["singleFloor"])
        shared_stacks = self._shared_core_stack_candidates(orientation_basis)
        independent_groups: list[
            tuple[FloorEnvironment, list[PlacementCandidate]]
        ] = []
        ...
        for environment in self.environments:
            ...
            if environment.done:
                ...
                independent_candidates = environment.generate_candidates(
                ...
                independent_candidates = environment.generate_candidates(
                    ...
                    allowed_categories={"corridor", "room", "special"},
                )
            if independent_candidates:
                environment.done = False
                independent_groups.append((environment, independent_candidates))
            elif not shared_stacks:
                environment.done = True

        if not independent_groups and not shared_stacks:
            return self._finish_episode()

        temperature = max(0.32, 0.90 * math.exp(-self.episode / 45.0))
        placements: list[dict] = []
        stack_decision: dict[str, Any] | None = None
        selected_stack: CoreStackCandidate | None = None

        # A shared Core is one building action, not one sampled floor action per
        # level.  Pool each stack's already floor-specific features, and include
        # one explicit no-stack branch only when independent room actions exist.
        # The retained log-probability therefore exactly matches the transition
        # that is committed, independent of floor count.
        if shared_stacks:
            building_actions: list[CoreStackCandidate | None] = []
            building_features: list[list[float]] = []
            if independent_groups:
                per_floor_non_core = [
                    _mean_feature_rows(
                        [candidate.features for candidate in candidates]
                    )
                    for _, candidates in independent_groups
                ]
                building_actions.append(None)
                building_features.append(_mean_feature_rows(per_floor_non_core))
            for stack in shared_stacks:
                building_actions.append(stack)
            ...
            building_tensor = torch.tensor(
                building_features, dtype=torch.float32, device=self.device
            )
            building_logits = torch.nan_to_num(
                self.model.placement_logits(building_tensor),
                nan=0.0,
                posinf=20.0,
                neginf=-20.0,
            ).clamp(-30.0, 30.0) / temperature
            building_distribution = torch.distributions.Categorical(
                logits=building_logits
            )
            building_index = building_distribution.sample()
            selected_stack = building_actions[int(building_index.item())]
            building_log_prob = building_distribution.log_prob(building_index)
            if selected_stack is None:
                self.placement_log_probs.append(building_log_prob)

        if selected_stack is not None:
            placements, stack_decision = self._commit_core_stack(
                selected_stack,
                trigger_floor=None,
                policy_log_prob=building_log_prob,
            )
            stack_decision.update(
                {
                    "policyCandidateCount": len(building_actions),
                    "noStackAvailable": bool(independent_groups),
                }
            )
        else:
            all_features = [
            ...
            cursor = 0
            for environment, candidates in independent_groups:
                ...
                candidate = candidates[int(selected_index.item())]
                log_prob = distribution.log_prob(selected_index)
                self.placement_log_probs.append(log_prob)
                placements.append(environment.place(candidate))
                if len(environment.placements) >= int(self.settings["maxModules"]):
                    environment.done = True
    ...
    def evaluate(self, generation_id: str, episode: int) -> dict[str, Any]:
        ...
        # Match terminal training exactly: eight points per post-BPE triangle,
        # averaged over floors.
        unmerged_triangles, unmerged_triangle_penalty = (
            _unmerged_triangle_metrics(layout_graphs)
        )
...
app = FastAPI(title="Module Lab v0.6-C")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
if __name__ == "__main__":
    print(f"Module Lab v0.6-C policy device: {select_device().type}")
