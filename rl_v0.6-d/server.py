...
class ParallelTrainer:
    ...
    def _sample_custom_shape(
        ...
        if module is None or (is_step0 and float(module["area"]) + 1.0e-8 < 24.0):
            large_edge_indices = [i for i, l in enumerate(G.EDGE_PALETTE) if 4.5 - 1e-4 <= l <= max_edge + 1e-4]
            if not large_edge_indices:
                large_edge_indices = [i for i, l in enumerate(G.EDGE_PALETTE) if l >= min_edge - 1e-4]
            e0 = large_edge_indices[(slot_index * 3) % len(large_edge_indices)]
            e1 = large_edge_indices[(slot_index * 5 + 2) % len(large_edge_indices)]
            e2 = large_edge_indices[(slot_index * 7 + 4) % len(large_edge_indices)]
            ang0 = 9 + (slot_index % 3)
            ang1 = 9 - (slot_index % 3)
            module = G.synthesize_custom_module(
                settings, category, 4, [e0, e1, e2], [ang0, ang1], f"s{slot_index}"
            )
...
app = FastAPI(title="Module Lab v0.6-D")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
