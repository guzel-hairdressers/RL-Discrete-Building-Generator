...
def single_polygon_signature(poly: list[dict], category: str = "room") -> str:
    """Return a rigid rotation-invariant signature for a single module polygon."""

    if not poly or len(poly) < 3:
        return "invalid"
    ccw_poly = _counter_clockwise_polygon(poly)
    candidates: list[tuple] = []
    for index, start in enumerate(ccw_poly):
        end = ccw_poly[(index + 1) % len(ccw_poly)]
        axis_length = distance(start, end)
        if axis_length > 1.0e-9:
            unit_x = (end["x"] - start["x"]) / axis_length
            unit_y = (end["y"] - start["y"]) / axis_length
            encoded_points = []
            for point in ccw_poly:
            ...
            candidates.append(_minimum_cyclic_tuple(encoded_points))
    if not candidates:
        return "invalid"
    return f"{category}:{json.dumps(min(candidates), separators=(',', ':'))}"
