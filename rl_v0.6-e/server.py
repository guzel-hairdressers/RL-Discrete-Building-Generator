...
class FloorEnvironment:
    ...
    def _candidate_from_anchor(
        ...
        # Hard Constraint: Reject candidate if ALL of its vertices lie strictly outside the outer site boundary
        outer_site = self.site["outer"]
        if all(not (G.point_in_polygon(vertex, outer_site) or G.point_on_polygon(vertex, outer_site)) for vertex in poly):
            return None
...
class ParallelTrainer:
    ...
    def _synthesize_dictionary(
        ...
        for slot_index in range(cap):
            category = "room" if single_floor else "shape"
            ...
            if slot_index == 0 and not single_floor:
                large_proposals = [p for p in proposals if float(p["area"]) >= 24.0]
                if large_proposals:
            ...
            canonical_mod = self._canonical_module(
                module, float(settings["angleStep"]), phase=episode + slot_index
            )
            dictionary.append(canonical_mod)
    ...
    def _evaluate_comprehensive_boundary_violations(self) -> dict[str, Any]:
        partial_crossings = 0
        complete_outside_count = 0
        site_enclosures_count = 0
        atrium_violations_count = 0

        for environment in self.environments:
            outer = environment.site["outer"]
            holes = environment.site.get("holes", []) or []
            for placement in environment.placements:
                poly = placement["poly"]
                if not poly:
                    continue

                # 1. Partial boundary crossing (not fully inside outer site or inside holes)
                if not G.polygon_inside_site(poly, outer, holes):
                    partial_crossings += 1

                # 2. Completely outside external boundary (all vertices strictly outside outer site)
                if all(not (G.point_in_polygon(vertex, outer) or G.point_on_polygon(vertex, outer)) for vertex in poly):
                    complete_outside_count += 1

                # 3. Outer site boundary consumption / enclosure (outer site completely inside placement poly)
                if len(poly) >= 3 and all(G.point_in_polygon(vertex, poly) or G.point_on_polygon(vertex, poly) for vertex in outer):
                    site_enclosures_count += 1

                # 4. Atrium hole violations (overlapping, inside, or enclosing internal atrium holes)
                for hole in holes:
                    if len(hole) >= 3:
                        if G.polygons_overlap(poly, hole) or G.polygon_inside_site(poly, hole):
                            atrium_violations_count += 1
                        elif all(G.point_in_polygon(vertex, poly) or G.point_on_polygon(vertex, poly) for vertex in hole):
                            atrium_violations_count += 1

        penalty_factor = 1.0 + 9.0 * min(1.0, self.episode / 100.0)
        weighted_violations = (
            partial_crossings
            + 3.0 * complete_outside_count
            + 5.0 * site_enclosures_count
            + 2.0 * atrium_violations_count
        )
        total_penalty = penalty_factor * weighted_violations

        return {
            "boundaryCrossingPenalty": total_penalty,
            "boundaryIntersections": partial_crossings,
            "completeOutsideCount": complete_outside_count,
            "siteEnclosuresCount": site_enclosures_count,
            "atriumViolationsCount": atrium_violations_count,
            "weightedBoundaryViolations": weighted_violations,
        }
    ...
    def _finish_episode(self) -> dict[str, Any]:
        ...
        bv_metrics = self._evaluate_comprehensive_boundary_violations()
        boundary_penalty = float(bv_metrics["boundaryCrossingPenalty"])
    ...
    def step(self, generation_id: Any, episode: Any) -> dict[str, Any]:
        ...
        for environment in self.environments:
            if environment.done:
                continue
            exact_site_area = max(1.0e-8, float(environment.site["exactArea"]))
            if len(environment.placements) >= int(self.settings["maxModules"]) or environment.filled_area >= exact_site_area - 1.0e-6:
    ...
    def evaluate(self, generation_id: str, episode: int) -> dict[str, Any]:
        ...
        score = float(metrics["score"]) + bpe_bonus - unmerged_triangle_penalty - boundary_penalty
        metrics["score"] = f"{score:.4f}"
        metrics.update(bv_metrics)
        metrics["vocabSize"] = bpe_stats["unique_types"]
        metrics["totalPlacements"] = bpe_stats["total_placements"]
...
app = FastAPI(title="Module Lab v0.6-E")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
