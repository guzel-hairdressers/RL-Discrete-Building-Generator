...
class DynamicPaletteTests(unittest.TestCase):
    ...
    def test_policy_dictionary_contains_parameter_actions_and_obeys_size_ratio(self) -> None:
        ...
        self.assertTrue(all(module["family"] == "dynamic-palette" for module in trainer.dictionary))
        self.assertTrue(all("palette" in module["parameters"] for module in trainer.dictionary))
        areas = [module["area"] for module in trainer.dictionary]
        self.assertLessEqual(max(areas) / min(areas), 5.0 + 1.0e-8)
        self.assertTrue(all(term.requires_grad for term in trainer.shape_log_probs))
        trapezoid = next(
            module
            for module in trainer.dictionary
            if module["parameters"]["palette"]["shapeType"] == "symmetric_trapezoid"
        ) if any(
            module["parameters"]["palette"]["shapeType"] == "symmetric_trapezoid"
            for module in trainer.dictionary
        ) else None
        if trapezoid is not None:
            self.assertGreaterEqual(len(trapezoid["rotations"]), 36)
    ...
    def test_terminal_learning_reaches_every_parameter_head(self) -> None:
        trainer = server.ParallelTrainer()
        trainer.update_settings(
            {
                "boundaryType": "rect",
        ...
        heads = (
            trainer.model.width_head,
            trainer.model.height_head,
            trainer.model.angle_head,
            trainer.model.shape_type_head,
        )
        before = [
        ...
        for prior_parameters, head in zip(before, heads):
            self.assertTrue(
                any(
                    not torch.equal(prior, current.detach())
                    for prior, current in zip(prior_parameters, head.parameters())
                )
            )
            self.assertTrue(
