...
def polygon_symmetry_order(poly: Sequence[dict]) -> int:
    """Return the rotational symmetry order of a polygon.

    Tests candidate orders in descending priority and returns the highest
    order *k* such that rotating the polygon by 360/k degrees produces an
    identical shape (matching ``_polygon_signature``).  Returns 1 when no
    rotational symmetry is detected.
    """

    base_signature = _polygon_signature(poly)
    for order in (12, 8, 6, 4, 3, 2):
        angle = 360.0 / order
        rotated = rotate_polygon(poly, angle)
        if _polygon_signature(rotated) == base_signature:
            return order
    return 1


def is_convex_polygon(poly: Sequence[dict]) -> bool:
    """Return True if *poly* is a strictly convex polygon (no reflex vertices)."""
    ...
    sign = None
    for i in range(n):
        x1 = poly[(i + 1) % n]["x"] - poly[i]["x"]
        y1 = poly[(i + 1) % n]["y"] - poly[i]["y"]
        x2 = poly[(i + 2) % n]["x"] - poly[(i + 1) % n]["x"]
        y2 = poly[(i + 2) % n]["y"] - poly[(i + 1) % n]["y"]
        cross = x1 * y2 - y1 * x2
        if abs(cross) < EPSILON:
            continue  # collinear vertices are acceptable
        if sign is None:
            sign = cross > 0
        elif (cross > 0) != sign:
            return False
...
def normalize_rotations(
    ...
    *phase* offsets that deterministic sample between training episodes.
    ...
    if not math.isfinite(step) or step <= EPSILON:
        ...
        # Exploit rotational symmetry to reduce the angle search range.
        # A polygon with k-fold symmetry only needs angles in [0, 360/k).
        sym_order = polygon_symmetry_order(poly)
        if sym_order > 1:
            total = max(1, total // sym_order)
        if total <= sample_cap:
...
def build_site(boundary: dict, holes: Sequence[Sequence[dict]] | None = None) -> dict:
    ...
    return {
        ...
        "isConvex": is_convex_polygon(outer),
