...
def worker_main(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for required in ("server.py", "geometry.py", "graph.py"):
        if not (module_dir / required).is_file():
            raise FileNotFoundError(f"{module_dir} is missing {required}")
    ...
    if imported_server != (module_dir / "server.py").resolve():
        raise RuntimeError(f"import contamination: loaded {imported_server}, expected {module_dir / 'server.py'}")
