...
_native_requested_off = (
    os.environ.get("MODULE_LAB_DISABLE_NATIVE_GEOMETRY", "").strip().lower()
    in _NATIVE_DISABLE_VALUES
)
_native_load_error: str | None = None
_native_library_path: str | None = None
_libfast_geo = None


def _native_library_candidates() -> list[str]:
    base = os.path.dirname(os.path.abspath(__file__))
    c_dir = os.path.join(base, "c")
    if sys.platform == "darwin":
        names = ("libfast_geometry.dylib", "libfast_geometry.so")
    elif sys.platform.startswith("linux"):
        names = ("libfast_geometry.so",)
    else:
        names = ()
    candidates = [os.path.join(c_dir, name) for name in names]
    candidates.extend([os.path.join(base, name) for name in names])
    return candidates


def _configure_native_library(library: ctypes.CDLL) -> None:
    ...
    if actual_abi != NATIVE_GEOMETRY_ABI_VERSION:
        raise RuntimeError(
            f"fast geometry ABI {actual_abi} does not match "
            f"required ABI {NATIVE_GEOMETRY_ABI_VERSION}"
        )
    library.polygons_overlap_c.argtypes = [
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
    ]
    library.polygons_overlap_c.restype = ctypes.c_int
    library.polygon_inside_site_c.argtypes = [
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
    ...
    library.polygon_inside_site_c.restype = ctypes.c_int
    library.get_shared_overlap_c.argtypes = [
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
    ]
    library.get_shared_overlap_c.restype = ctypes.c_double
    library.shared_overlap_pair_c.argtypes = [
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
    ...
    library.shared_overlap_pair_c.restype = None
    library.symmetric_segment_overlap_c.argtypes = [
        _CPoint,
        _CPoint,
        _CPoint,
        _CPoint,
    ...
    library.symmetric_segment_overlap_c.restype = ctypes.c_int
    library.point_to_segments_distance_c.argtypes = [
        _CPoint,
        ctypes.POINTER(_CPoint),
        ctypes.c_int,
    ]
...
for _candidate_path in _native_candidate_paths:
    if not os.path.isfile(_candidate_path):
        continue
    try:
        _candidate_library = ctypes.CDLL(_candidate_path)
        _configure_native_library(_candidate_library)
        _libfast_geo = _candidate_library
        _native_library_path = _candidate_path
        _native_load_error = None
        break
    except (OSError, AttributeError, RuntimeError) as error:
        _native_load_error = f"{type(error).__name__}: {error}"

if _libfast_geo is None and _native_load_error is None:
    if _native_candidate_paths:
        _native_load_error = "native geometry library was not built"
    else:
        _native_load_error = f"native geometry is unsupported on {sys.platform}"

NATIVE_GEOMETRY_AVAILABLE = _libfast_geo is not None
NATIVE_GEOMETRY_ENABLED = NATIVE_GEOMETRY_AVAILABLE and not _native_requested_off


def set_native_geometry_enabled(enabled: bool) -> bool:
    """Enable or disable native dispatch, returning the resulting state.

    Disabling is useful for differential tests and benchmarks. Enabling fails
    loudly if a compatible platform library was not loaded at import time.
    """

    global NATIVE_GEOMETRY_ENABLED
    requested = bool(enabled)
    if requested and not NATIVE_GEOMETRY_AVAILABLE:
        raise RuntimeError(_native_load_error or "native geometry library is unavailable")
    NATIVE_GEOMETRY_ENABLED = requested
    return NATIVE_GEOMETRY_ENABLED


def native_geometry_status() -> dict[str, object]:
    """Return stable, JSON-safe native-kernel diagnostics."""

    return {
        "available": NATIVE_GEOMETRY_AVAILABLE,
        "enabled": NATIVE_GEOMETRY_ENABLED,
        "abiVersion": NATIVE_GEOMETRY_ABI_VERSION,
        "library": _native_library_path,
        "loadError": _native_load_error,
        "disabledByEnvironment": _native_requested_off,
    }
...
def _packed_polygon_from_signature(signature: _NativePolygonSignature):
    point_array = _CPoint * len(signature)
    return point_array(*(_CPoint(x, y) for x, y in signature))


@functools.lru_cache(maxsize=512)
def _packed_holes_from_signatures(
    signatures: tuple[_NativePolygonSignature, ...],
):
    counts_array = ctypes.c_int * len(signatures)
    counts = counts_array(*(len(signature) for signature in signatures))
    flattened = tuple(point for signature in signatures for point in signature)
    points_array = _CPoint * len(flattened)
    points = points_array(*(_CPoint(x, y) for x, y in flattened))
    return points, counts


def _native_polygons_overlap(
    first_poly: Sequence[dict], second_poly: Sequence[dict]
) -> bool:
    first_signature = _native_polygon_signature(first_poly)
    second_signature = _native_polygon_signature(second_poly)
    first_points = _packed_polygon_from_signature(first_signature)
    second_points = _packed_polygon_from_signature(second_signature)
    return bool(
        _libfast_geo.polygons_overlap_c(
            first_points,
            len(first_signature),
            second_points,
...
def _native_polygon_inside_site(
    poly: Sequence[dict],
    outer: Sequence[dict],
    holes: Sequence[Sequence[dict]],
) -> bool:
    poly_signature = _native_polygon_signature(poly)
    outer_signature = _native_polygon_signature(outer)
    hole_signatures = tuple(_native_polygon_signature(hole) for hole in holes)
    poly_points = _packed_polygon_from_signature(poly_signature)
    outer_points = _packed_polygon_from_signature(outer_signature)
    if hole_signatures:
        hole_points, hole_counts = _packed_holes_from_signatures(hole_signatures)
    else:
        hole_points = None
        hole_counts = None
    return bool(
        _libfast_geo.polygon_inside_site_c(
            poly_points,
            len(poly_signature),
            outer_points,
            len(outer_signature),
...
def _native_shared_overlap_pair_from_signatures(
    first_signature: _NativePolygonSignature,
    second_signature: _NativePolygonSignature,
) -> tuple[float, float]:
    first_points = _packed_polygon_from_signature(first_signature)
    second_points = _packed_polygon_from_signature(second_signature)
    ...
    return float(maximum.value), float(total.value)


def _native_shared_overlap_pair(
    first_poly: Sequence[dict], second_poly: Sequence[dict]
) -> tuple[float, float]:
    return _native_shared_overlap_pair_from_signatures(
        _native_polygon_signature(first_poly),
        _native_polygon_signature(second_poly),
    )
...
def _native_symmetric_segment_overlap_values(
    ...
    second_end = ctypes.c_double(0.0)
    shared_length = ctypes.c_double(0.0)
    found = _libfast_geo.symmetric_segment_overlap_c(
        _CPoint(first_x, first_y),
        _CPoint(second_x, second_y),
        _CPoint(third_x, third_y),
        _CPoint(fourth_x, fourth_y),
        linear_tolerance,
        angular_tolerance,
        ctypes.byref(first_start),
        ctypes.byref(first_end),
        ctypes.byref(second_start),
        ctypes.byref(second_end),
        ctypes.byref(shared_length),
    )
    if not found:
        return None
    return (
        (float(first_start.value), float(first_end.value)),
...
def _symmetric_segment_overlap_python(
    ...
    first_length = segment_length(first, second)
    second_length = segment_length(third, fourth)
    if first_length <= 1.0e-5 or second_length <= 1.0e-5:
        return None
    dx1 = second["x"] - first["x"]
    dy1 = second["y"] - first["y"]
    dx2 = fourth["x"] - third["x"]
    dy2 = fourth["y"] - third["y"]
    if abs(dx1 * dy2 - dy1 * dx2) / (first_length * second_length) > angular_tolerance:
        return None

    def orient(start: dict, end: dict, point: dict) -> float:
        return (
    ...
    if not (
        abs(orient(first, second, third)) <= linear_tolerance * first_length
        and abs(orient(first, second, fourth)) <= linear_tolerance * first_length
        and abs(orient(third, fourth, first)) <= linear_tolerance * second_length
        and abs(orient(third, fourth, second)) <= linear_tolerance * second_length
    ):
        return None

    def interval(start: dict, end: dict, other_start: dict, other_end: dict):
        dx = end["x"] - start["x"]
        dy = end["y"] - start["y"]
        ...
        overlap_start = max(0.0, min(start_parameter, end_parameter))
        overlap_end = min(1.0, max(start_parameter, end_parameter))
        if (overlap_end - overlap_start) * math.sqrt(length_squared) <= linear_tolerance:
            return None
        return overlap_start, overlap_end

    first_interval = interval(first, second, third, fourth)
    second_interval = interval(third, fourth, first, second)
...
def symmetric_segment_overlap(
    first: dict,
    second: dict,
    third: dict,
    fourth: dict,
    linear_tolerance: float = 1.0e-2,
    angular_tolerance: float = 1.0e-3,
) -> tuple[tuple[float, float], tuple[float, float], float] | None:
    """Measure tolerant collinear overlap in both segment frames."""

    if NATIVE_GEOMETRY_ENABLED:
        return _native_symmetric_segment_overlap_values(
            float(first["x"]),
            float(first["y"]),
            float(second["x"]),
            float(second["y"]),
    ...
    return _symmetric_segment_overlap_python(
        first,
        second,
        third,
        fourth,
...
# v0.6-B keeps geometry proposals discrete.  The policy selects indices from
# these palettes instead of selecting a pre-baked shape ID or emitting raw
# vertices. A half-metre edge grid plus standard sqrt(3) diagonal members
# covers the project-wide 1m..9m limits; active settings mask values outside
# the requested range.
EDGE_PALETTE = tuple(
    sorted(
        {1.0 + 0.5 * index for index in range(17)}
        | {math.sqrt(3.0) * multiplier for multiplier in range(1, 6)}
    )
)
ANGLE_PALETTE = tuple(float(angle) for angle in range(45, 136, 5))
PARAMETRIC_SHAPE_TYPES = (
    "rectangle",
    "parallelogram",
    "symmetric_trapezoid",
    "irregular_quad",
...
def _polygons_overlap_python(
    first_poly: Sequence[dict], second_poly: Sequence[dict]
) -> bool:
    """Pure-Python reference for positive-interior polygon overlap.
...
def polygons_overlap(first_poly: Sequence[dict], second_poly: Sequence[dict]) -> bool:
    """Return whether two polygons share positive interior area.

    Native dispatch is semantics-compatible with the Python reference: ordinary
    wall/vertex contact remains non-overlap, while containment and identical
    polygons overlap.
    """

    if NATIVE_GEOMETRY_ENABLED:
        return _native_polygons_overlap(first_poly, second_poly)
    return _polygons_overlap_python(first_poly, second_poly)


def _polygon_inside_site_python(
    poly: Sequence[dict],
    outer: Sequence[dict],
    holes: Sequence[Sequence[dict]] | None = None,
) -> bool:
    """Pure-Python reference for exact site containment.
    ...
    for index, first in enumerate(poly):
        ...
        for start, end in zip(parameters, parameters[1:]):
            ...
            if not (point_in_polygon(sample, outer) or point_on_polygon(sample, outer)):
                return False
    return not any(_polygons_overlap_python(poly, hole) for hole in holes)


def polygon_inside_site(
    poly: Sequence[dict],
    outer: Sequence[dict],
    holes: Sequence[Sequence[dict]] | None = None,
) -> bool:
    ...
    Full edges are split at every outer-boundary intersection in both native and
    fallback implementations; testing only vertices or one midpoint is not
    sufficient for concave sites.
    """

    normalized_holes = holes or []
    if NATIVE_GEOMETRY_ENABLED:
        return _native_polygon_inside_site(poly, outer, normalized_holes)
    return _polygon_inside_site_python(poly, outer, normalized_holes)


def _shared_overlap_pair_python(
    first_poly: Sequence[dict], second_poly: Sequence[dict]
) -> tuple[float, float]:
    """Return ``(longest, total)`` from one Python edge-pair traversal."""
    ...
    return longest, math.fsum(sorted(overlaps))


def shared_overlap_pair(
    first_poly: Sequence[dict], second_poly: Sequence[dict]
) -> tuple[float, float]:
    """Return longest and total coincident boundary length in one kernel call."""

    if NATIVE_GEOMETRY_ENABLED:
        return _native_shared_overlap_pair(first_poly, second_poly)
    return _shared_overlap_pair_python(first_poly, second_poly)
...
def get_shared_overlap(first_poly: Sequence[dict], second_poly: Sequence[dict]) -> float:
    ...
    return shared_overlap_pair(first_poly, second_poly)[1]
...
def max_shared_overlap(first_poly: Sequence[dict], second_poly: Sequence[dict]) -> float:
    ...
    return shared_overlap_pair(first_poly, second_poly)[0]
...
def _packed_segments_from_signature(
    signature: tuple[tuple[float, float, float, float], ...],
):
    point_array = _CPoint * (len(signature) * 2)
    return point_array(
        *(
            _CPoint(x, y)
            for first_x, first_y, second_x, second_y in signature
            for x, y in ((first_x, first_y), (second_x, second_y))
        )
    )


def _point_to_segment_signature_dist_python(
    point: dict,
    signature: tuple[tuple[float, float, float, float], ...],
) -> float:
    best = math.inf
    ...
    for first_x, first_y, second_x, second_y in signature:
        ...
        length_squared = dx * dx + dy * dy
        if length_squared <= EPSILON:
            distance = math.hypot(point_x - first_x, point_y - first_y)
        else:
            parameter = clamp(
                ((point_x - first_x) * dx + (point_y - first_y) * dy) / length_squared,
                0.0,
                1.0,
            )
            distance = math.hypot(
                point_x - (first_x + parameter * dx),
                point_y - (first_y + parameter * dy),
            )
        best = min(best, distance)
    return best


def _point_to_segments_dist_python(point: dict, segments: Iterable) -> float:
    return _point_to_segment_signature_dist_python(point, _segment_signature(segments))
    ...
    if NATIVE_GEOMETRY_ENABLED and signature:
        ...
        return float(
            _libfast_geo.point_to_segments_distance_c(
                _CPoint(float(point["x"]), float(point["y"])),
                packed,
                len(signature),
            )
        )
    return _point_to_segment_signature_dist_python(point, signature)
...
def rotation_only_polygon_signature(poly: Sequence[dict], grid: float = 1.0e-6) -> str:
    """Canonicalize translation, cyclic start, and rigid rotation—not reflection."""

    points = [_copy_point(point) for point in poly]
    if len(points) < 3:
        raise ValueError("polygon signature requires at least three vertices")
    if polygon_signed_area(points) < 0.0:
        points.reverse()
    candidates = []
    for start_index, origin in enumerate(points):
        axis_end = points[(start_index + 1) % len(points)]
        axis_length = _distance(origin, axis_end)
        ...
        for offset in range(len(points)):
            point = points[(start_index + offset) % len(points)]
            relative_x = point["x"] - origin["x"]
            relative_y = point["y"] - origin["y"]
            encoded.append(
                (
                    int(round((relative_x * unit_x + relative_y * unit_y) / grid)),
                    int(round((-relative_x * unit_y + relative_y * unit_x) / grid)),
                )
            )
        candidates.append(tuple(encoded))
    if not candidates:
        raise ValueError("polygon signature requires nonzero edges")
    return json.dumps(min(candidates), separators=(",", ":"))


def active_angle_indices(settings: dict) -> list[int]:
    """Return global angle-palette indices allowed by ``angleStep``.

    Internal angles remain on the common five-degree lattice so independently
    proposed modules can still share exact ports.  ``angleStep == 0`` disables
    placement rotations, while internal geometry keeps the safe 15-degree
    palette (needed for valid three-edge cores).
    """

    step = float(settings.get("angleStep", 15.0))
    if step <= EPSILON:
        step = 15.0
    indices = [
        index
        for index, angle in enumerate(ANGLE_PALETTE)
        if abs(angle / step - round(angle / step)) <= 1.0e-8
    ]
    return indices or [ANGLE_PALETTE.index(90.0)]


def _parametric_polygon(shape_type: str, width: float, height: float, angle: float) -> list[dict]:
    """Construct a clean triangle or quad from palette values."""

    theta = math.radians(angle)
    run = height * math.cos(theta)
    rise = height * math.sin(theta)
    if shape_type == "rectangle":
        return [_point(0.0, 0.0), _point(width, 0.0), _point(width, height), _point(0.0, height)]
    if shape_type == "parallelogram":
        return [
            _point(0.0, 0.0),
            _point(width, 0.0),
            _point(width + run, rise),
            _point(run, rise),
        ]
    if shape_type == "symmetric_trapezoid":
        return [
            _point(0.0, 0.0),
            _point(width, 0.0),
            _point(width - run, rise),
    ...
    if shape_type == "irregular_quad":
        # Chiral closed chain with headings 0, alpha, 180-alpha, 270,
        # where alpha is half the selected internal-angle parameter. Solving
        # closure determines the last two lengths. Standard sqrt(3) palette
        # entries make the 60-degree family exact (e.g. sqrt(3), 2, 4, 3).
        alpha = theta / 2.0
        cosine = math.cos(alpha)
        sine = math.sin(alpha)
        third_length = height + width / cosine
        return [
            _point(0.0, 0.0),
            _point(width, 0.0),
            _point(width + height * cosine, height * sine),
            _point(
                width + height * cosine - third_length * cosine,
                height * sine + third_length * sine,
            ),
        ]
    if shape_type == "triangle":
        return [_point(0.0, 0.0), _point(width, 0.0), _point(run, rise)]
...
def _cached_enumerate_parametric_proposals(
    ...
    for type_index, shape_type in enumerate(PARAMETRIC_SHAPE_TYPES):
        ...
        for width_index in edge_indices:
            width = EDGE_PALETTE[width_index]
            for height_index in edge_indices:
                height = EDGE_PALETTE[height_index]
                for angle_index in angle_indices:
                    angle = ANGLE_PALETTE[angle_index]
                    poly = _parametric_polygon(shape_type, width, height, angle)
                    if not _valid_generated_module_polygon(poly, minimum, maximum, True):
                        continue
                    edges = _edge_lengths(poly)
                    angles = internal_angles(poly)
                    if any(
                        ...
                        continue
                    if any(
                        abs(internal_angle / active_step - round(internal_angle / active_step)) > 1.0e-6
                        for internal_angle in angles
                    ):
                        continue
                    area = polygon_area(poly)
                    if category == "core" and not 20.0 - EPSILON <= area <= 30.0 + EPSILON:
                        continue
                    if category != "core" and area < 4.0 - EPSILON:
                        continue
                    if category == "corridor" and min_polygon_width(poly) > 1.5 + COLLINEAR_EPSILON:
                        continue
                    geometry_signature = rotation_only_polygon_signature(poly)
...
def enumerate_parametric_proposals(settings: dict, category: str) -> list[dict]:
    ...
    return [
        copy.deepcopy(prop)
        for prop in _cached_enumerate_parametric_proposals(
            category, minimum, maximum, max_edges, active_step
        )
    ]
...
def synthesize_parametric_module(
    ...
    """Create one module from a policy-selected discrete parameter vector."""

    try:
        width = EDGE_PALETTE[int(width_index)]
        height = EDGE_PALETTE[int(height_index)]
        angle = ANGLE_PALETTE[int(angle_index)]
        shape_type = PARAMETRIC_SHAPE_TYPES[int(type_index)]
    except (IndexError, TypeError, ValueError) as error:
        raise ValueError("palette action index is out of range") from error
    proposal_key = (int(width_index), int(height_index), int(angle_index), int(type_index))
    allowed = {
        (item["widthIndex"], item["heightIndex"], item["angleIndex"], item["typeIndex"])
        for item in enumerate_parametric_proposals(settings, category)
    }
    if proposal_key not in allowed:
        raise ValueError("palette action violates active geometry constraints")
    poly = _parametric_polygon(shape_type, width, height, angle)
    geometry_signature = rotation_only_polygon_signature(poly)
    identity_material = f"{str(category).lower()}|{geometry_signature}"
    identity_digest = hashlib.sha256(identity_material.encode("utf-8")).hexdigest()[:16]
    signature = f"P_{str(category).lower()}_{identity_digest}"
    record = _module_record(
        identifier=identifier,
        name=f"Dynamic {shape_type.replace('_', ' ').title()}",
        category=str(category).lower(),
        poly=poly,
        family="dynamic-palette",
        edge_range_compatible=True,
        source_parameters={
            "generator": "discrete-palette",
            "shapeType": signature,
            "palette": {
                "widthIndex": int(width_index),
                "heightIndex": int(height_index),
                "angleIndex": int(angle_index),
                "typeIndex": int(type_index),
                "width": width,
                "height": height,
                "angle": angle,
                "angleStep": float(settings.get("angleStep", 15.0)),
                "shapeType": shape_type,
                "geometrySignature": geometry_signature,
            },
        },
    )
    record["shapeType"] = signature
    record["parameters"]["latent"] = {
        "input": [
            width_index / max(1, len(EDGE_PALETTE) - 1),
            height_index / max(1, len(EDGE_PALETTE) - 1),
            angle_index / max(1, len(ANGLE_PALETTE) - 1),
            type_index / max(1, len(PARAMETRIC_SHAPE_TYPES) - 1),
            clamp(record["area"] / 40.0, 0.0, 1.0),
            1.0 if str(category).lower() == "core" else 0.0,
            clamp(float(settings.get("angleStep", 15.0)) / 90.0, 0.0, 1.0),
            1.0,
        ]
    }
    return record
...
def synthesize_custom_polygon(
    num_edges: int,
    edge_lengths: list[float],
    internal_angles: list[float],
) -> list[dict]:
    """Construct a closed polygon of k edges (strictly k=3 triangles or k=4 quads) using specified lengths and angles."""
    k = int(num_edges)
    if k not in (3, 4):
        raise ValueError("Custom polygon must be either a triangle (k=3) or a quad (k=4)")
    
    current_alpha = 0.0
    points = [_point(0.0, 0.0)]
    
    l1 = edge_lengths[0]
    points.append(_point(l1, 0.0))
    ...
    for i in range(2, k):
        ...
        raw_angle = internal_angles[i - 2]
        if k == 4 and i == 3:
            # For 4-gons, constrain 2nd internal angle to maintain convex/non-self-intersecting quad geometry
            target_angle = 180.0 - internal_angles[0]
            raw_angle = clamp(raw_angle, max(45.0, target_angle - 30.0), min(135.0, target_angle + 30.0))
        theta = math.radians(raw_angle)
        
        # Turn left
        current_alpha = (current_alpha + math.pi - theta) % (2.0 * math.pi)
        
        current_x += length * math.cos(current_alpha)
        current_y += length * math.sin(current_alpha)
        points.append(_point(current_x, current_y))
        
    return translate_to_origin(points)


def synthesize_custom_module(
    settings: dict,
    category: str,
    num_edges: int,
    edge_length_indices: list[int],
    internal_angle_indices: list[int],
    identifier: str,
) -> dict:
    """Create one module from a policy-selected discrete parameter vector."""
    k = int(num_edges)
    edge_lengths = [EDGE_PALETTE[int(idx)] for idx in edge_length_indices[:k-1]]
    internal_angles = [ANGLE_PALETTE[int(idx)] for idx in internal_angle_indices[:k-2]]
    ...
    all_lengths = _edge_lengths(poly)
    if any(l < min_edge - 1e-4 or l > max_edge + 1e-4 for l in all_lengths):
        raise ValueError(f"Generated polygon edge length violates minEdge ({min_edge}m) or maxEdge ({max_edge}m) limits")
    ...
    record = _module_record(
        ...
        source_parameters={
            "numEdges": k,
            "edgeLengthIndices": edge_length_indices[:k-1],
            "internalAngleIndices": internal_angle_indices[:k-2],
        },
    )
    record["shapeType"] = signature
    
    # Pad to represent as latent input
    latent_input = [
        k / 8.0,
        (edge_length_indices[0] if len(edge_length_indices) > 0 else 0) / max(1, len(EDGE_PALETTE) - 1),
        (internal_angle_indices[0] if len(internal_angle_indices) > 0 else 0) / max(1, len(ANGLE_PALETTE) - 1),
        0.0,
        clamp(record["area"] / 40.0, 0.0, 1.0),
        1.0 if str(category).lower() == "core" else 0.0,
        clamp(float(settings.get("angleStep", 15.0)) / 90.0, 0.0, 1.0),
    ...
    record["parameters"]["latent"] = {"input": latent_input}
    return record
...
def generate_basic_dictionary(settings: dict) -> list[dict]:
    ...
    Each shape is defined by standard coordinates, with edges on the grid and angles
    being exact multiples of 15 degrees.
    """
    # Shape definitions
    shapes = [
        # Quads - Rectangles
    ...
    allow_corridors = bool(settings.get("allowCorridors", False))
    filtered_shapes = [
        s for s in shapes 
        if len(s["poly"]) <= max_edges and (allow_corridors or s["category"] != "corridor")
    ]
