...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_root_spec_geometry_control_limits_match_server_contract(self) -> None:
        elements_by_id = {
            attrs["id"]: attrs
            for _, attrs in self.parser.elements
            if attrs.get("id")
        }
        for identifier in ("minEdge", "minEdgeNum", "maxEdge", "maxEdgeNum"):
    ...
    def test_single_floor_and_triangle_score_copy_match_backend_semantics(self) -> None:
        self.assertIn("Room-only palette; disable core topology", self.html)
        self.assertIn("metrics.topologyEnabled !== false", self.app)
        self.assertIn("Disabled in canonical single-floor mode", self.app)
