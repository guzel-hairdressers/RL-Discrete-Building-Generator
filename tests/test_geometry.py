...
class ProceduralGeometryTests(unittest.TestCase):
    ...
    def test_fixed_dictionary_filters_actual_edges_and_vertex_count(self) -> None:
        settings = {
            "minEdge": 4.0,
            "maxEdge": 9.0,
            "maxEdges": 3,
        ...
        dictionary = G.generate_basic_dictionary(settings)

        self.assertTrue(dictionary)
        self.assertEqual({module["category"] for module in dictionary}, {"core", "room"})
        for module in dictionary:
            with self.subTest(module=module["id"]):
                edge_lengths = module["parameters"]["edgeLengths"]
                self.assertLessEqual(len(module["poly"]), settings["maxEdges"])
                self.assertGreaterEqual(min(edge_lengths), settings["minEdge"] - 1.0e-8)
                self.assertLessEqual(max(edge_lengths), settings["maxEdge"] + 1.0e-8)
                self.assertTrue(module["edgeRangeCompatible"])

        infeasible = G.generate_basic_dictionary(
            {"minEdge": 8.5, "maxEdge": 9.0, "maxEdges": 8}
        )
