...
class SettingsTests(unittest.TestCase):
    ...
    def test_root_spec_geometry_limits_are_hard_bounds(self) -> None:
        for patch in (
            {"minEdge": 0.5},
            {"minEdge": 9.5},
            {"maxEdge": 0.5},
        ...
        valid = server.validate_settings_patch(
            server.DEFAULT_SETTINGS,
            {"minEdge": 1.0, "maxEdge": 9.0, "maxEdges": 3},
        )
        self.assertEqual((valid["minEdge"], valid["maxEdge"], valid["maxEdges"]), (1.0, 9.0, 3))

    def test_infeasible_fixed_palette_rejects_atomically(self) -> None:
        ...
        settings_before = trainer.settings
        dictionary_before = trainer.dictionary
        environments_before = trainer.environments
        generation_before = trainer.generation_id

        with self.assertRaisesRegex(server.SettingsError, "no feasible"):
            trainer.update_settings({"minEdge": 8.5, "maxEdge": 9.0})

        self.assertIs(trainer.settings, settings_before)
...
class ParallelTrainerTests(unittest.TestCase):
    ...
    def test_single_floor_dictionary_first_placement_and_metrics_are_room_only(self) -> None:
        trainer = server.ParallelTrainer()
        ...
        self.assertEqual({module["category"] for module in site["dictionary"]}, {"room"})
        self.assertNotIn("core", {module["category"] for module in trainer.dictionary})
        self.assertNotIn("corridor", {module["category"] for module in trainer.dictionary})

        first = trainer.step(trainer.generation_id, trainer.episode)
        self.assertEqual(len(first["placements"]), 1)
        self.assertEqual(first["placements"][0]["module"]["category"], "room")

        multiplier_before = trainer.topology_multiplier
        per_site = [
            environment.terminal_metrics(True, float(trainer.settings["coreSpacing"]))
            for environment in trainer.environments
        ]
        metrics = trainer._aggregate_terminal(per_site)
        self.assertFalse(metrics["topologyEnabled"])
        self.assertTrue(metrics["topologyValid"])
        self.assertEqual(metrics["topologyViolations"], [])
        self.assertEqual(metrics["topologyViolationRate"], 0.0)
        self.assertEqual(metrics["topologyPenalty"], 0.0)
        self.assertEqual(metrics["topologyMultiplier"], 0.0)
        self.assertEqual(trainer.topology_multiplier, multiplier_before)
    ...
    def test_triangle_penalty_is_exactly_eight_times_average_per_floor(self) -> None:
        triangle = [
            {"x": 0.0, "y": 0.0},
            {"x": 4.0, "y": 0.0},
            {"x": 0.0, "y": 4.0},
        ...
        graphs = [
            server.graph.LayoutGraph(
                nodes={
                    "a": {"id": "a", "poly": triangle},
                    "b": {"id": "b", "poly": G.translate_polygon(triangle, 10.0, 0.0)},
                },
                connections=[],
                playground_id=0,
            ),
            server.graph.LayoutGraph(
                nodes={"c": {"id": "c", "poly": G.translate_polygon(triangle, 20.0, 0.0)}},
                connections=[],
                playground_id=1,
            ),
        ]

        total, average, penalty = server._unmerged_triangle_summary(graphs)
