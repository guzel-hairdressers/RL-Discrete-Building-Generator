...
def create_3d_context_visualization(scene, output_path=None):
    ...
    if output_path is None:
        site_id_str = scene.get("site_id", "context")
        output_path = os.path.join(os.path.dirname(__file__), "output", f"{site_id_str}_3d.html")
...
function createArchitecturalVolume(verts, faces, colorHex = 0xffffff, opacity = 1.0, isRoad = false) {{
  ...
  const selfShadowColor = new THREE.Color(0xe0e0e0); // 100% Pure Monochrome Grey (R=224, G=224, B=224) - Zero Blue Tint!
